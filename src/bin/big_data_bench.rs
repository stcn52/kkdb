...
fn main() {
    ...
    println!("Starting ingestion of {} rows in batches of {}...", total_rows, batch_size);
    ...
    for batch in 0..(total_rows / batch_size) {
        ...
        if (batch + 1) % 100 == 0 {
            ...
            println!("  Inserted {:>8} / {} rows ({:>8.0} rows/sec)", rows_inserted, total_rows, rows_per_sec);
        }
    }
    ...
    println!("Average speed: {:.0} rows/sec", total_rows as f64 / total_time);
    ...
    vm.execute_sql("SELECT * FROM huge_table WHERE id = 5000000").unwrap();
    println!("Point lookup (id=5000000) took: {:.2?}", q_start.elapsed());
    
    let q_start = Instant::now();
    vm.execute_sql("SELECT * FROM huge_table WHERE id = 9999999").unwrap();
    println!("Point lookup (id=9999999) took: {:.2?}", q_start.elapsed());
    
    // Get file sizes
    if let Ok(metadata) = std::fs::metadata(db_path) {
        println!("Final DB file size: {:.2} MB", metadata.len() as f64 / 1_048_576.0);
    }
    if let Ok(metadata) = std::fs::metadata(format!("{}.binlog", db_path)) {
        println!("Final Binlog file size: {:.2} MB", metadata.len() as f64 / 1_048_576.0);
    }
