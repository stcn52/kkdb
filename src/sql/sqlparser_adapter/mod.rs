...
fn try_parse_create_vector_index(sql: &str) -> Option<Result<kk::Statement>> {
    ...
    // Must start with CREATE
    if tokens.first().map(|t| t.to_ascii_uppercase() != "CREATE").unwrap_or(true) {
        return None;
    }
    let mut idx = 1;

    // Must have VECTOR
    if tokens.get(idx).map(|t| t.to_ascii_uppercase() != "VECTOR").unwrap_or(true) {
        return None;
    }
    idx += 1;
    ...
    if tokens.get(idx).map(|t| t.to_ascii_uppercase() == "IF").unwrap_or(false) {
        idx += 1;
        if tokens.get(idx).map(|t| t.to_ascii_uppercase() == "NOT").unwrap_or(false) { idx += 1; }
        if tokens.get(idx).map(|t| t.to_ascii_uppercase() == "EXISTS").unwrap_or(false) { idx += 1; }
        if_not_exists = true;
    }
    ...
    let index_name = match tokens.get(idx) {
        Some(t) => t.trim_matches(|c: char| !c.is_alphanumeric() && c != '_').to_string(),
        None => return Some(Err(KkdbError::ParseError("VECTOR INDEX missing name".into()))),
    };
    idx += 1;

    // ON
    if tokens.get(idx).map(|t| t.to_ascii_uppercase() != "ON").unwrap_or(true) {
        return Some(Err(KkdbError::ParseError("VECTOR INDEX missing ON clause".into())));
    }
    idx += 1;
    ...
    let table_name_raw = match tokens.get(idx) {
        Some(t) => *t,
        None => return Some(Err(KkdbError::ParseError("VECTOR INDEX missing table name".into()))),
    };
    let table_name = if let Some(pos) = table_name_raw.find('(') {
        table_name_raw[..pos].to_string()
    } else {
    ...
    let column = {
        ...
        match (op, cp) {
            (Some(o), Some(c)) if c > o => {
                after_on[o + 1..c].trim().trim_matches('`').trim_matches('"').to_string()
            }
            _ => return Some(Err(KkdbError::ParseError("VECTOR INDEX missing (column)".into()))),
        }
    };
    ...
    // DIM N
    let dim: u32 = if tokens.get(idx).map(|t| t.to_ascii_uppercase() == "DIM").unwrap_or(false) {
        idx += 1;
        match tokens.get(idx).and_then(|t| {
            ...
            Some(n) => { idx += 1; n }
            None => return Some(Err(KkdbError::ParseError("VECTOR INDEX DIM must be a positive integer".into()))),
        }
    } else {
        return Some(Err(KkdbError::ParseError("VECTOR INDEX requires DIM N clause".into())));
    };

    // Optional DISTANCE COSINE|L2
    let distance = if tokens.get(idx).map(|t| t.to_ascii_uppercase() == "DISTANCE").unwrap_or(false) {
        idx += 1;
        match tokens.get(idx).map(|t| t.to_ascii_uppercase()).as_deref() {
            ...
            other => return Some(Err(KkdbError::ParseError(
                format!("unknown distance metric {:?}; use COSINE or L2", other)
            ))),
        }
    } else {
    ...
    Some(Ok(kk::Statement::CreateVectorIndex(kk::CreateVectorIndexStmt {
        index_name,
        table_name: table_name.trim_matches('`').to_string(),
        column,
        dim,
