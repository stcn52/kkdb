...
fn test_param_insert_or_replace_with_special_chars() {
    ...
    assert!(result.is_ok(), "execute_params INSERT OR REPLACE failed: {:?}", result);
...
fn test_param_insert_or_replace_with_fts() {
    ...
    vm.execute_sql(
        "CREATE FULLTEXT INDEX idx_skills_fts ON skills (desc, tags, prompts)"
    ).unwrap();
    ...
    assert!(result.is_ok(), "INSERT OR REPLACE + FTS failed: {:?}", result);
