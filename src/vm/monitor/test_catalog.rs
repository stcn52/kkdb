...
impl TestCatalog {
    ...
    pub fn build_full() -> Self {
        ...
        cat.add(
            "basic.rs",
            TestCategory::Core,
            vec![TestTag::Sql, TestTag::Unit],
            "VM construction, CREATE/DROP, INSERT basics",
        );
        cat.add(
            "select.rs",
            TestCategory::Core,
            vec![TestTag::Sql, TestTag::Dml],
        ...
        cat.add(
            "eval_expr_coverage.rs",
            TestCategory::Expression,
            vec![TestTag::Sql, TestTag::Unit],
            "Expression evaluator coverage",
        );
        cat.add(
            "sqlparser_expr_coverage.rs",
            TestCategory::Expression,
            vec![TestTag::Sql, TestTag::Unit],
        ...
        cat.add(
            ...
            vec![TestTag::Mvcc, TestTag::Integration],
            "Transaction commit/rollback, isolation",
        );
        cat.add(
            "mvcc_visibility.rs",
            TestCategory::Transaction,
            vec![TestTag::Mvcc, TestTag::Unit],
        ...
        cat.add(
            "select_for_update.rs",
            TestCategory::Transaction,
            vec![TestTag::Mvcc, TestTag::Sql],
            "SELECT FOR UPDATE",
        );
        cat.add(
            "read_committed.rs",
        ...
        cat.add(
            "query_cache_integration.rs",
            TestCategory::Feature,
            vec![TestTag::Performance, TestTag::Integration],
            "Query cache integration",
        );
        cat.add(
            "binlog_coverage.rs",
            TestCategory::Feature,
            vec![TestTag::Storage, TestTag::Unit],
            "Binlog coverage",
        );
        cat.add(
            "raft_coverage.rs",
            TestCategory::Feature,
            vec![TestTag::Raft, TestTag::Integration],
            "Raft consensus coverage",
        );

        // Round coverage R8-R18
        cat.add(
            "coverage_r8_optimizer_wal.rs",
            TestCategory::CoverageRounds,
            vec![TestTag::Optimizer, TestTag::Storage],
            "R8: optimizer + WAL",
        );
        cat.add(
            "coverage_r9_mvcc_fts_raft.rs",
            TestCategory::CoverageRounds,
            vec![TestTag::Mvcc, TestTag::FullText, TestTag::Raft],
            "R9: MVCC + FTS + Raft",
        );
        cat.add(
            "coverage_r10_prepared_bloom_wf.rs",
            TestCategory::CoverageRounds,
            vec![TestTag::Sql, TestTag::Storage],
            "R10: prepared statements + bloom + WF",
        );
        cat.add(
            "coverage_r18_adv_query.rs",
            TestCategory::CoverageRounds,
            vec![TestTag::Sql, TestTag::Optimizer],
...
mod tests {
    ...
    fn test_category_directory() {
        ...
        assert_eq!(
            TestCategory::CoverageRounds.directory(),
            "execute_tests/rounds"
        );
    }
