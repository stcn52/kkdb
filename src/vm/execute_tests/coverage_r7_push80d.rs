...
fn test_vacuum_after_heavy_deletes() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE vac2(id INTEGER PRIMARY KEY, data TEXT)");
    for i in 1..=100 {
        exec(&mut vm, &format!("INSERT INTO vac2 VALUES ({i}, '{}')", "x".repeat(50)));
    }
    for i in 1..=80 {
...
fn test_cast_text_to_real() {
    ...
    if let Value::Real(v) = rows[0][0] { assert!((v - 3.14).abs() < 0.01); }
}

#[test]
fn test_between_symmetric() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE bsym(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "INSERT INTO bsym VALUES (1, 5), (2, 15), (3, 25), (4, 35)");
...
fn test_not_between() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE nbw(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_match_against_basic() {
    ...
    exec(&mut vm, "INSERT INTO ma VALUES (1, 'hello world', 'this is test content')");
    exec(&mut vm, "INSERT INTO ma VALUES (2, 'goodbye moon', 'another document here')");
    exec(&mut vm, "INSERT INTO ma VALUES (3, 'hello again', 'more hello content')");
    // MATCH AGAINST without FTS index — uses fallback scoring
    let r = try_exec(&mut vm, "SELECT id, MATCH(title, body) AGAINST ('hello') AS score FROM ma");
...
fn test_match_against_multi_word() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ma2(id INTEGER PRIMARY KEY, content TEXT)");
    exec(&mut vm, "INSERT INTO ma2 VALUES (1, 'rust programming language')");
    exec(&mut vm, "INSERT INTO ma2 VALUES (2, 'python data science')");
    exec(&mut vm, "INSERT INTO ma2 VALUES (3, 'rust embedded systems')");
    let r = try_exec(&mut vm, "SELECT id, MATCH(content) AGAINST ('rust programming') AS score FROM ma2");
    let _ = r;
}
...
fn test_ilike_case_insensitive() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ilk(id INTEGER PRIMARY KEY, val TEXT)");
...
fn test_json_keys_empty_object() {
    ...
    if let Value::Text(v) = &rows[0][0] {
        assert!(v.as_ref() == "[]" || v.as_ref() == "{}", "expected [] or {{}}, got {v}");
    }
}
...
fn test_select_for_update_all_rows() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE sfu2(id INTEGER PRIMARY KEY, val TEXT)");
...
fn test_check_constraint_int_vs_real() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ckr(id INTEGER PRIMARY KEY, val REAL CHECK(val > 0))");
...
fn test_insert_or_replace_unique_conflict() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE uq(id INTEGER PRIMARY KEY, email TEXT UNIQUE, name TEXT)");
    ...
    exec(&mut vm, "INSERT OR REPLACE INTO uq VALUES (3, 'a@b.com', 'Charlie')");
...
fn test_insert_null_columns() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE inull(id INTEGER PRIMARY KEY, a TEXT, b INTEGER, c REAL)");
...
fn test_explain_subquery() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE es(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "INSERT INTO es VALUES (1, 10)");
    let r = try_exec(&mut vm, "EXPLAIN SELECT * FROM (SELECT id, val FROM es) AS sub");
...
fn test_explain_join_tree() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ej1(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE TABLE ej2(id INTEGER PRIMARY KEY, ref_id INTEGER)");
    exec(&mut vm, "INSERT INTO ej1 VALUES (1, 10), (2, 20)");
    exec(&mut vm, "INSERT INTO ej2 VALUES (1, 1), (2, 2)");
    let r = try_exec(&mut vm, "EXPLAIN SELECT * FROM ej1 JOIN ej2 ON ej1.id = ej2.ref_id");
...
fn test_cross_join() {
    ...
    exec(&mut vm, "CREATE TABLE cj2(id INTEGER PRIMARY KEY, name TEXT)");
    exec(&mut vm, "INSERT INTO cj1 VALUES (1, 'A'), (2, 'B')");
    exec(&mut vm, "INSERT INTO cj2 VALUES (1, 'X'), (2, 'Y'), (3, 'Z')");
...
fn test_natural_join() {
    ...
    exec(&mut vm, "CREATE TABLE nj2(id INTEGER PRIMARY KEY, extra TEXT)");
...
fn test_analyze_and_between_cbo() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE cbo(id INTEGER PRIMARY KEY, val INTEGER)");
    for i in 1..=100 {
...
fn test_o3_auto_index_threshold() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ai(id INTEGER PRIMARY KEY, category TEXT, val INTEGER)");
    for i in 1..=20 {
        exec(&mut vm, &format!("INSERT INTO ai VALUES ({i}, 'cat_{}'  , {i})", i % 5));
    }
    // Keep querying WHERE category = ... to exceed adaptive threshold (5)
    for i in 0..8 {
        let _ = query_rows(&mut vm, &format!("SELECT * FROM ai WHERE category = 'cat_{}'", i % 5));
    }
...
fn test_unsupported_create_function() {
    let mut vm = VM::new_memory();
    let r = try_exec(&mut vm, "CREATE FUNCTION my_func() RETURNS INTEGER BEGIN RETURN 1; END");
...
fn test_json_arrow_access() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE js(id INTEGER PRIMARY KEY, data TEXT)");
    exec(&mut vm, "INSERT INTO js VALUES (1, '{\"name\": \"Alice\", \"age\": 30}')");
...
fn test_json_double_arrow_access() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE js2(id INTEGER PRIMARY KEY, data TEXT)");
    exec(&mut vm, "INSERT INTO js2 VALUES (1, '{\"info\": {\"key\": \"val\"}}')");
...
fn test_nested_union_with_order_by() {
    let mut vm = VM::new_memory();
    let rows = query_rows(&mut vm, "SELECT 3 AS v UNION SELECT 1 UNION SELECT 2 ORDER BY v");
...
fn test_union_with_limit() {
    let mut vm = VM::new_memory();
    let rows = query_rows(&mut vm, "SELECT 1 AS v UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 LIMIT 2");
    assert_eq!(rows.len(), 2);
}
...
fn test_percent_rank_detailed() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE prd(id INTEGER PRIMARY KEY, grp TEXT, score INTEGER)");
...
fn test_cume_dist_detailed() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE cdd(id INTEGER PRIMARY KEY, grp TEXT, score INTEGER)");
...
fn test_dense_rank_with_ties_detailed() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE drd(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_fts_match_with_index() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE docs(id INTEGER PRIMARY KEY, title TEXT, body TEXT)");
    exec(&mut vm, "INSERT INTO docs VALUES (1, 'rust programming', 'rust is a systems language')");
    exec(&mut vm, "INSERT INTO docs VALUES (2, 'python tutorial', 'python for data science')");
    exec(&mut vm, "INSERT INTO docs VALUES (3, 'database design', 'sql and nosql databases')");
    exec(&mut vm, "INSERT INTO docs VALUES (4, 'rust web', 'building web apps with rust')");
    exec(&mut vm, "INSERT INTO docs VALUES (5, 'javascript guide', 'nodejs and browser js')");
    // Create fulltext index
    let r = try_exec(&mut vm, "CREATE FULLTEXT INDEX idx_docs_fts ON docs(title, body)");
    if r.is_ok() {
        // FTS MATCH query through the inverted index
        let r2 = try_exec(&mut vm, "SELECT * FROM docs WHERE MATCH(title, body) AGAINST ('rust')");
        let _ = r2;
    }
}
...
fn test_fts_delete_with_index() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ftsd(id INTEGER PRIMARY KEY, content TEXT)");
...
fn test_order_by_limit_top_n_large() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE topn3(id INTEGER PRIMARY KEY, score INTEGER)");
    for i in 1..=500 {
        exec(&mut vm, &format!("INSERT INTO topn3 VALUES ({i}, {})", 500 - i));
    }
...
fn test_order_by_limit_offset_large() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE topn4(id INTEGER PRIMARY KEY, val INTEGER)");
    for i in 1..=300 {
        exec(&mut vm, &format!("INSERT INTO topn4 VALUES ({i}, {i})"));
    }
    let rows = query_rows(&mut vm, "SELECT id FROM topn4 ORDER BY val LIMIT 5 OFFSET 10");
...
fn test_order_by_limit_zero() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE topn5(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_drop_regular_index() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE dri(id INTEGER PRIMARY KEY, val TEXT)");
...
fn test_vm_directory_mode() {
    ...
    exec(&mut vm, "CREATE TABLE dir_t(id INTEGER PRIMARY KEY, val TEXT)");
...
fn test_schema_restore_from_file() {
    ...
    // Create and populate
    {
        ...
        exec(&mut vm, "CREATE TABLE sr(id INTEGER PRIMARY KEY, val TEXT, age INTEGER CHECK(age > 0))");
...
fn test_schema_restore_fts_index() {
    ...
    {
        let mut vm = VM::open(path).unwrap();
        exec(&mut vm, "CREATE TABLE fts_t(id INTEGER PRIMARY KEY, content TEXT)");
...
fn test_schema_restore_multi_table() {
    ...
    {
        let mut vm = VM::open(path).unwrap();
        exec(&mut vm, "CREATE TABLE mt1(id INTEGER PRIMARY KEY, val TEXT)");
        exec(&mut vm, "CREATE TABLE mt2(id INTEGER PRIMARY KEY, ref_id INTEGER, name TEXT)");
...
fn test_schema_restore_fk_constraints() {
    ...
    {
        let mut vm = VM::open(path).unwrap();
        exec(&mut vm, "CREATE TABLE parent(id INTEGER PRIMARY KEY, name TEXT)");
...
fn test_lz4_compression_via_set() {
    let mut vm = VM::new_memory();
    let _ = try_exec(&mut vm, "SET use_lz4 = 'on'");
    exec(&mut vm, "CREATE TABLE lz4t(id INTEGER PRIMARY KEY, data TEXT)");
    for i in 1..=30 {
        exec(&mut vm, &format!("INSERT INTO lz4t VALUES ({i}, '{}')", "data".repeat(100)));
    }
...
fn test_buffer_pool_small_eviction() {
    ...
    exec(&mut vm, "CREATE TABLE evt(id INTEGER PRIMARY KEY, data TEXT)");
    // Insert enough data to exceed 8 pages and trigger eviction
    for i in 1..=100 {
        exec(&mut vm, &format!("INSERT INTO evt VALUES ({i}, '{}')", "x".repeat(200)));
    }
...
fn test_insert_multiple_values_auto_txn() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE mul(id INTEGER PRIMARY KEY, val TEXT)");
    exec(&mut vm, "INSERT INTO mul VALUES (1, 'a'), (2, 'b'), (3, 'c'), (4, 'd'), (5, 'e')");
...
fn test_insert_values_large_batch() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE batch(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_insert_or_replace_pk_conflict() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ior(id INTEGER PRIMARY KEY, name TEXT, score INTEGER)");
    exec(&mut vm, "INSERT INTO ior VALUES (1, 'Alice', 100)");
    exec(&mut vm, "INSERT INTO ior VALUES (2, 'Bob', 200)");
    // PK conflict on id=1 — replace
    exec(&mut vm, "INSERT OR REPLACE INTO ior VALUES (1, 'Alice_v2', 150)");
...
fn test_insert_or_replace_multiple_conflicts() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ior2(id INTEGER PRIMARY KEY, val TEXT)");
    exec(&mut vm, "INSERT INTO ior2 VALUES (1, 'a'), (2, 'b'), (3, 'c')");
    // Replace all three
...
fn test_insert_or_replace_with_check() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE iorc(id INTEGER PRIMARY KEY, val INTEGER CHECK(val > 0))");
...
fn test_overflow_cell_large_insert() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ov(id INTEGER PRIMARY KEY, data TEXT)");
...
fn test_overflow_cell_multiple() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ov2(id INTEGER PRIMARY KEY, data TEXT)");
    for i in 1..=5 {
...
fn test_scan_all_overflow_rows() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE sov(id INTEGER PRIMARY KEY, data TEXT)");
    for i in 1..=3 {
...
fn test_query_cache_hit_miss() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE qc(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_fts_legacy_virtual_table_match() {
    ...
    let r = try_exec(&mut vm, "CREATE TABLE ftsvt(id INTEGER PRIMARY KEY, content TEXT) FULLTEXT");
    if r.is_ok() {
        exec(&mut vm, "INSERT INTO ftsvt VALUES (1, 'hello world test')");
        exec(&mut vm, "INSERT INTO ftsvt VALUES (2, 'goodbye world test')");
...
fn test_not_in_list() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ni(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_create_index_multi_column() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE mc(id INTEGER PRIMARY KEY, a TEXT, b INTEGER, c TEXT)");
...
fn test_all_window_funcs_together() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE awf(id INTEGER PRIMARY KEY, grp TEXT, val INTEGER)");
    for i in 1..=8 {
        exec(&mut vm, &format!("INSERT INTO awf VALUES ({i}, '{}', {})", 
            if i <= 4 { "A" } else { "B" }, i * 10));
    }
    let rows = query_rows(&mut vm,
        "SELECT id, val, \
...
fn test_nth_value_in_partition() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE nvp(id INTEGER PRIMARY KEY, grp TEXT, val INTEGER)");
...
fn test_having_with_aggregation() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE hv(id INTEGER PRIMARY KEY, category TEXT, amount INTEGER)");
    exec(&mut vm, "INSERT INTO hv VALUES (1, 'A', 10), (2, 'A', 20), (3, 'B', 5), (4, 'B', 100), (5, 'C', 1)");
    let rows = query_rows(&mut vm, "SELECT category, SUM(amount) AS total FROM hv GROUP BY category HAVING SUM(amount) > 10");
...
fn test_case_when_complex() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE cw(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "INSERT INTO cw VALUES (1, 10), (2, 20), (3, 30), (4, 40), (5, 50)");
    let rows = query_rows(&mut vm,
...
fn test_string_agg_or_concat() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE gc(id INTEGER PRIMARY KEY, grp TEXT, val TEXT)");
    exec(&mut vm, "INSERT INTO gc VALUES (1, 'A', 'x'), (2, 'A', 'y'), (3, 'B', 'z')");
    // GROUP_CONCAT not supported; test concatenation via || operator
    let rows = query_rows(&mut vm, "SELECT grp, COUNT(val) FROM gc GROUP BY grp ORDER BY grp");
    assert_eq!(rows.len(), 2);
...
fn test_order_by_desc() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE obd(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "INSERT INTO obd VALUES (1, 10), (2, 50), (3, 30), (4, 20), (5, 40)");
...
fn test_multiple_aggregates() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE magg(id INTEGER PRIMARY KEY, val INTEGER)");
    for i in 1..=10 {
...
fn test_union_all() {
    let mut vm = VM::new_memory();
    let rows = query_rows(&mut vm, "SELECT 1 AS v UNION ALL SELECT 1 UNION ALL SELECT 2");
...
fn test_except() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ex1(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE TABLE ex2(id INTEGER PRIMARY KEY, val INTEGER)");
