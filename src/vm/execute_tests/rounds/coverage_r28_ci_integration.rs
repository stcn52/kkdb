...
mod tests {
    ...
    fn test_explain_select() {
        ...
        x(
            &mut vm,
            "CREATE TABLE ex1 (id INTEGER PRIMARY KEY, name TEXT, age INTEGER)",
        );
        x(&mut vm, "INSERT INTO ex1 VALUES (1, 'Alice', 30)");

        let result = vm
            .execute_sql("EXPLAIN SELECT * FROM ex1 WHERE id = 1")
            .unwrap();
        match result {
            ExecResult::Explain { plan } => {
                ...
                assert!(
                    ...
                    "EXPLAIN plan should mention scan or select: {}",
                    plan
                );
            }
    ...
    fn test_explain_analyze_select() {
        ...
        x(
            &mut vm,
            "CREATE TABLE ex2 (id INTEGER PRIMARY KEY, val REAL)",
        );
        for i in 1..=10 {
            x(
                &mut vm,
                &format!("INSERT INTO ex2 VALUES ({}, {})", i, i as f64 * 1.5),
            );
        }

        let result = vm
            .execute_sql("EXPLAIN ANALYZE SELECT * FROM ex2 WHERE val > 5.0")
            .unwrap();
        match result {
            ExecResult::Explain { plan } => {
    ...
    fn test_explain_format_tree() {
        ...
        x(
            &mut vm,
            "CREATE TABLE ex3 (id INTEGER PRIMARY KEY, name TEXT)",
        );
        x(&mut vm, "INSERT INTO ex3 VALUES (1, 'test')");

        // SQLite dialect: EXPLAIN FORMAT TREE (no parentheses)
        let result = vm
            .execute_sql("EXPLAIN FORMAT TREE SELECT * FROM ex3 WHERE id = 1 ORDER BY name LIMIT 5")
            .unwrap();
        match result {
            ExecResult::Explain { plan } => {
                assert!(plan.contains("TREE"), "Should contain TREE: {}", plan);
                assert!(
                    plan.contains("SELECT") || plan.contains("└") || plan.contains("├"),
    ...
    fn test_explain_format_json() {
        ...
        x(
            &mut vm,
            "CREATE TABLE ex4 (id INTEGER PRIMARY KEY, data TEXT)",
        );
        x(&mut vm, "INSERT INTO ex4 VALUES (1, 'json_test')");

        // SQLite dialect: EXPLAIN FORMAT JSON (no parentheses)
        let result = vm
            .execute_sql("EXPLAIN FORMAT JSON SELECT * FROM ex4 WHERE id = 1")
            .unwrap();
        match result {
            ExecResult::Explain { plan } => {
                assert!(
                    plan.contains("{") || plan.contains("node_type") || plan.contains("\""),
    ...
    fn test_explain_with_join() {
        ...
        x(
            &mut vm,
            "CREATE TABLE ex_a (id INTEGER PRIMARY KEY, name TEXT)",
        );
        x(
            &mut vm,
            "CREATE TABLE ex_b (id INTEGER PRIMARY KEY, a_id INTEGER, val TEXT)",
        );
        x(&mut vm, "INSERT INTO ex_a VALUES (1, 'A')");
        x(&mut vm, "INSERT INTO ex_b VALUES (1, 1, 'B')");

        let result = vm
            .execute_sql("EXPLAIN SELECT a.name, b.val FROM ex_a a JOIN ex_b b ON a.id = b.a_id")
            .unwrap();
        match result {
            ExecResult::Explain { plan } => {
    ...
    fn test_grant_revoke() {
        ...
        x(
            &mut vm,
            "CREATE TABLE grant_tbl (id INTEGER PRIMARY KEY, data TEXT)",
        );
    ...
    fn test_show_engine_status() {
        ...
        x(
            &mut vm,
            "CREATE TABLE status_tbl (id INTEGER PRIMARY KEY, v TEXT)",
        );
        for i in 1..=5 {
            x(
                &mut vm,
                &format!("INSERT INTO status_tbl VALUES ({}, 'val{}')", i, i),
            );
        }
        ...
        match result {
            ExecResult::Explain { plan } => {
                assert!(
                    plan.contains("Buffer pool") || plan.contains("buffer"),
                    "Should show buffer pool info: {}",
                    plan
                );
                assert!(
                    plan.contains("WAL") || plan.contains("wal"),
                    "Should show WAL info: {}",
                    plan
                );
            }
    ...
    fn test_not_exists_subquery() {
        ...
        x(
            &mut vm,
            "CREATE TABLE sq_orders (id INTEGER PRIMARY KEY, cust_id INTEGER)",
        );
        x(
            &mut vm,
            "CREATE TABLE sq_custs (id INTEGER PRIMARY KEY, name TEXT)",
        );
    ...
    fn test_in_subquery() {
        ...
        x(
            &mut vm,
            "CREATE TABLE in_main (id INTEGER PRIMARY KEY, val TEXT)",
        );
        x(
            &mut vm,
            "CREATE TABLE in_ref (id INTEGER PRIMARY KEY, main_id INTEGER)",
        );
    ...
    fn test_nested_subquery() {
        ...
        x(
            &mut vm,
            "CREATE TABLE nest_a (id INTEGER PRIMARY KEY, val INTEGER)",
        );
    ...
    fn test_scalar_subquery() {
        ...
        x(
            &mut vm,
            "CREATE TABLE sc_items (id INTEGER PRIMARY KEY, price REAL)",
        );
    ...
    fn test_error_unknown_table() {
        ...
        assert!(
            err_msg.contains("nonexistent_table")
                ...
                || err_msg.to_lowercase().contains("no such"),
            "Error should reference the table: {}",
            err_msg
        );
    }
    ...
    fn test_error_unknown_column() {
        ...
        x(
            &mut vm,
            "CREATE TABLE err_col (id INTEGER PRIMARY KEY, name TEXT)",
        );
    ...
    fn test_error_duplicate_insert() {
        ...
        x(
            &mut vm,
            "CREATE TABLE dup_tbl (id INTEGER PRIMARY KEY, v TEXT)",
        );
    ...
    fn test_error_check_constraint_violation() {
        ...
        x(
            &mut vm,
            "CREATE TABLE chk_tbl (id INTEGER PRIMARY KEY, age INTEGER CHECK(age >= 0))",
        );
    ...
    fn test_error_not_null_violation() {
        ...
        x(
            &mut vm,
            "CREATE TABLE nn_tbl (id INTEGER PRIMARY KEY, name TEXT NOT NULL)",
        );
    ...
    fn test_window_row_number() {
        ...
        x(
            &mut vm,
            "CREATE TABLE wf1 (id INTEGER PRIMARY KEY, dept TEXT, salary INTEGER)",
        );
    ...
    fn test_window_rank_dense_rank() {
        ...
        x(
            &mut vm,
            "CREATE TABLE wf2 (id INTEGER PRIMARY KEY, score INTEGER)",
        );
    ...
    fn test_window_sum_avg() {
        ...
        x(
            &mut vm,
            "CREATE TABLE wf3 (id INTEGER PRIMARY KEY, grp TEXT, val INTEGER)",
        );
    ...
    fn test_transaction_commit() {
        ...
        x(
            &mut vm,
            "CREATE TABLE txn1 (id INTEGER PRIMARY KEY, val TEXT)",
        );
    ...
    fn test_transaction_rollback() {
        ...
        x(
            &mut vm,
            "CREATE TABLE txn2 (id INTEGER PRIMARY KEY, val TEXT)",
        );
        ...
        x(
            &mut vm,
            "INSERT INTO txn2 VALUES (2, 'will_be_rolled_back')",
        );
        x(
            &mut vm,
            "INSERT INTO txn2 VALUES (3, 'will_be_rolled_back')",
        );
        x(&mut vm, "ROLLBACK");
    ...
    fn test_savepoint_and_rollback_to() {
        ...
        x(
            &mut vm,
            "CREATE TABLE txn3 (id INTEGER PRIMARY KEY, val TEXT)",
        );
    ...
    fn test_create_and_query_view() {
        ...
        x(
            &mut vm,
            "CREATE TABLE view_base (id INTEGER PRIMARY KEY, name TEXT, active INTEGER)",
        );
    ...
    fn test_view_with_aggregation() {
        ...
        x(
            &mut vm,
            "CREATE TABLE view_sales (id INTEGER PRIMARY KEY, product TEXT, amount REAL)",
        );
        x(
            &mut vm,
            "INSERT INTO view_sales VALUES (1, 'Widget', 100.0)",
        );
        x(
            &mut vm,
            "INSERT INTO view_sales VALUES (2, 'Widget', 200.0)",
        );
        x(
            &mut vm,
            "INSERT INTO view_sales VALUES (3, 'Gadget', 150.0)",
        );
        ...
        let rows = qr(
            &mut vm,
            "SELECT product, total FROM sales_summary ORDER BY product",
        );
        assert_eq!(rows.len(), 2);
    }
    ...
    fn test_trigger_after_insert() {
        ...
        x(
            &mut vm,
            "CREATE TABLE trig_main (id INTEGER PRIMARY KEY, name TEXT)",
        );
        x(
            &mut vm,
            "CREATE TABLE trig_log (id INTEGER PRIMARY KEY AUTOINCREMENT, event TEXT)",
        );
    ...
    fn test_check_constraint_pass() {
        ...
        x(
            &mut vm,
            "CREATE TABLE chk_ok (id INTEGER PRIMARY KEY, age INTEGER CHECK(age >= 18))",
        );
    ...
    fn test_analyze_table() {
        ...
        x(
            &mut vm,
            "CREATE TABLE analyze_tbl (id INTEGER PRIMARY KEY, name TEXT, age INTEGER)",
        );
        for i in 1..=20 {
            x(
                &mut vm,
                &format!(
                    "INSERT INTO analyze_tbl VALUES ({}, 'user{}', {})",
                    i,
                    i,
                    20 + i
                ),
    ...
    fn test_vacuum() {
        ...
        x(
            &mut vm,
            "CREATE TABLE vac_tbl (id INTEGER PRIMARY KEY, data TEXT)",
        );
        for i in 1..=10 {
            x(
                &mut vm,
                &format!("INSERT INTO vac_tbl VALUES ({}, 'data{}')", i, i),
            );
        }
    ...
    fn test_multiple_ctes() {
        ...
        x(
            &mut vm,
            "CREATE TABLE multi_cte (id INTEGER PRIMARY KEY, category TEXT, value INTEGER)",
        );
    ...
    fn test_union_all() {
        ...
        x(
            &mut vm,
            "CREATE TABLE ua_a (id INTEGER PRIMARY KEY, v TEXT)",
        );
        x(
            &mut vm,
            "CREATE TABLE ua_b (id INTEGER PRIMARY KEY, v TEXT)",
        );
        x(&mut vm, "INSERT INTO ua_a VALUES (1, 'X')");
        x(&mut vm, "INSERT INTO ua_b VALUES (1, 'X')");

        let rows = qr(&mut vm, "SELECT v FROM ua_a UNION ALL SELECT v FROM ua_b");
    ...
    fn test_json_type() {
        ...
        x(
            &mut vm,
            "CREATE TABLE jt2 (id INTEGER PRIMARY KEY, data TEXT)",
        );
        x(&mut vm, "INSERT INTO jt2 VALUES (1, '{\"key\": 42}')");

        let rows = qr(&mut vm, "SELECT JSON_TYPE(data) FROM jt2 WHERE id = 1");
        assert_eq!(rows.len(), 1);
        let val = rows[0][0].to_string();
        assert!(
            val.contains("object") || val.contains("Object") || val.contains("OBJECT"),
    ...
    fn test_large_insert_and_aggregate() {
        ...
        x(
            &mut vm,
            "CREATE TABLE large_tbl (id INTEGER PRIMARY KEY, val INTEGER, grp TEXT)",
        );
    ...
    fn test_three_way_join() {
        ...
        x(
            &mut vm,
            "CREATE TABLE j_orders (id INTEGER PRIMARY KEY, user_id INTEGER, product_id INTEGER)",
        );
        x(
            &mut vm,
            "CREATE TABLE j_products (id INTEGER PRIMARY KEY, pname TEXT, price REAL)",
        );
        ...
        x(
            &mut vm,
            "INSERT INTO j_products VALUES (1, 'Widget', 29.99)",
        );
        x(
            &mut vm,
            "INSERT INTO j_products VALUES (2, 'Gadget', 49.99)",
        );
    ...
    fn test_left_join_with_nulls() {
        ...
        x(
            &mut vm,
            "CREATE TABLE lj_a (id INTEGER PRIMARY KEY, name TEXT)",
        );
        x(
            &mut vm,
            "CREATE TABLE lj_b (id INTEGER PRIMARY KEY, a_id INTEGER, info TEXT)",
        );
        ...
        assert!(
            rows[1][1].to_string() == "NULL"
                || rows[1][1].to_string() == "null"
                || rows[1][1].to_string().is_empty(),
            "Bob's info should be NULL: {}",
            rows[1][1]
        );
    }
    ...
    fn test_case_expression() {
        ...
        x(
            &mut vm,
            "CREATE TABLE case_tbl (id INTEGER PRIMARY KEY, score INTEGER)",
        );
    ...
    fn test_coalesce() {
        ...
        x(
            &mut vm,
            "CREATE TABLE coal_tbl (id INTEGER PRIMARY KEY, a TEXT, b TEXT)",
        );
        x(&mut vm, "INSERT INTO coal_tbl VALUES (1, NULL, 'fallback')");
        x(
            &mut vm,
            "INSERT INTO coal_tbl VALUES (2, 'value', 'fallback')",
        );

        let rows = qr(&mut vm, "SELECT COALESCE(a, b) FROM coal_tbl ORDER BY id");
        assert_eq!(rows.len(), 2);
