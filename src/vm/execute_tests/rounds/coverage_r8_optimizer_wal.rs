...
fn test_dp_join_reorder_2_tables() {
    ...
    for i in 0..50 {
        exec(
            &mut vm,
            &format!("INSERT INTO big VALUES ({}, 'b{}')", i, i),
        )
        .unwrap();
    }
    for i in 0..5 {
        exec(
            &mut vm,
            &format!("INSERT INTO small VALUES ({}, 's{}')", i, i),
        )
        .unwrap();
    }
    ...
    let r = rows(
        &mut vm,
        "SELECT big.id, small.val FROM big INNER JOIN small ON big.id = small.id",
    );
    assert_eq!(r.len(), 5);
}
...
fn test_dp_join_reorder_3_tables() {
    ...
    for i in 0..100 {
        exec(&mut vm, &format!("INSERT INTO t1 VALUES ({}, 'a')", i)).unwrap();
    }
    for i in 0..10 {
        exec(&mut vm, &format!("INSERT INTO t2 VALUES ({}, 'b')", i)).unwrap();
    }
    for i in 0..50 {
        exec(&mut vm, &format!("INSERT INTO t3 VALUES ({}, 'c')", i)).unwrap();
    }
...
fn test_dp_join_reorder_preserves_left_join() {
    ...
    for i in 0..10 {
        exec(&mut vm, &format!("INSERT INTO a VALUES ({})", i)).unwrap();
    }
    for i in 0..5 {
        exec(&mut vm, &format!("INSERT INTO b VALUES ({})", i)).unwrap();
    }
...
fn test_dp_join_equal_cardinalities_no_reorder() {
    ...
    for i in 0..20 {
        exec(&mut vm, &format!("INSERT INTO eq1 VALUES ({})", i)).unwrap();
    }
    for i in 0..20 {
        exec(&mut vm, &format!("INSERT INTO eq2 VALUES ({})", i)).unwrap();
    }
    ...
    let r = rows(
        &mut vm,
        "SELECT eq1.id FROM eq1 INNER JOIN eq2 ON eq1.id = eq2.id",
    );
    assert_eq!(r.len(), 20);
}
...
fn test_cbo_estimate_from_cardinality_with_stats() {
    ...
    exec(
        &mut vm,
        "CREATE TABLE stats_t (id INTEGER PRIMARY KEY, val TEXT)",
    )
    .unwrap();
    for i in 0..75 {
...
fn test_cbo_table_has_index_on_pk() {
    ...
    exec(
        &mut vm,
        "CREATE TABLE idx_test (id INTEGER PRIMARY KEY, name TEXT)",
    )
    .unwrap();
...
fn test_cbo_table_has_index_on_secondary() {
    ...
    exec(
        &mut vm,
        "CREATE TABLE idx2 (id INTEGER PRIMARY KEY, name TEXT)",
    )
    .unwrap();
    ...
    let _ = has_name_idx; // Just exercises the path
}
...
fn test_cbo_estimate_scan_cost() {
    ...
    exec(
        &mut vm,
        "CREATE TABLE scan_test (id INTEGER PRIMARY KEY, val TEXT)",
    )
    .unwrap();
...
fn test_join_selectivity_with_stats() {
    ...
    exec(
        &mut vm,
        "CREATE TABLE sel_a (id INTEGER PRIMARY KEY, grp INTEGER)",
    )
    .unwrap();
    exec(
        &mut vm,
        "CREATE TABLE sel_b (id INTEGER PRIMARY KEY, grp INTEGER)",
    )
    .unwrap();
    ...
    for i in 0..100 {
        exec(
            &mut vm,
            &format!("INSERT INTO sel_a VALUES ({}, {})", i, i % 10),
        )
        .unwrap();
    }
    for i in 0..50 {
        exec(
            &mut vm,
            &format!("INSERT INTO sel_b VALUES ({}, {})", i, i % 5),
        )
        .unwrap();
    }
...
fn test_join_selectivity_no_stats_defaults() {
    ...
    for i in 0..10 {
        exec(&mut vm, &format!("INSERT INTO ns_a VALUES ({})", i)).unwrap();
    }
    for i in 0..10 {
        exec(&mut vm, &format!("INSERT INTO ns_b VALUES ({})", i)).unwrap();
    }

    // No ANALYZE → uses default selectivity (10%)
    let r = rows(
        &mut vm,
        "SELECT ns_a.id FROM ns_a INNER JOIN ns_b ON ns_a.id = ns_b.id",
    );
    assert_eq!(r.len(), 10);
}
...
fn test_join_selectivity_cross_join() {
    ...
    for i in 0..3 {
        exec(&mut vm, &format!("INSERT INTO cj_a VALUES ({})", i)).unwrap();
    }
    for i in 0..4 {
        exec(&mut vm, &format!("INSERT INTO cj_b VALUES ({})", i)).unwrap();
    }
...
fn test_wal_checkpoint_blocked_by_snapshot() {
    ...
    assert_eq!(
        applied, 0,
        "checkpoint should be blocked by active snapshot"
    );
...
fn test_wal_group_commit_config_auto_sync() {
    use crate::storage::pager::PAGE_SIZE;
    use crate::storage::wal::{GroupCommitConfig, Wal, WalSyncMode};
    ...
    assert_eq!(
        stats.pending_sync_commits, 0,
        "auto-sync should have flushed"
    );
...
fn test_wal_group_commit_config_getter() {
    use crate::storage::wal::{GroupCommitConfig, Wal};
...
fn test_explain_shows_join_info() {
    ...
    exec(
        &mut vm,
        "CREATE TABLE ex2 (id INTEGER PRIMARY KEY, val TEXT)",
    )
    .unwrap();
    ...
    let result = exec(
        &mut vm,
        "EXPLAIN SELECT ex1.id FROM ex1 INNER JOIN ex2 ON ex1.id = ex2.id",
    );
...
fn test_4_table_join_with_reorder() {
    ...
    for t in &["j1", "j2", "j3", "j4"] {
        exec(
            &mut vm,
            &format!("CREATE TABLE {} (id INTEGER PRIMARY KEY, val TEXT)", t),
        )
        .unwrap();
    }
    ...
    for i in 0..200 {
        exec(&mut vm, &format!("INSERT INTO j1 VALUES ({}, 'a')", i)).unwrap();
    }
    for i in 0..5 {
        exec(&mut vm, &format!("INSERT INTO j2 VALUES ({}, 'b')", i)).unwrap();
    }
    for i in 0..50 {
        exec(&mut vm, &format!("INSERT INTO j3 VALUES ({}, 'c')", i)).unwrap();
    }
    for i in 0..20 {
        exec(&mut vm, &format!("INSERT INTO j4 VALUES ({}, 'd')", i)).unwrap();
    }
...
fn test_join_with_index_on_pk() {
    ...
    exec(
        &mut vm,
        "CREATE TABLE pk_a (id INTEGER PRIMARY KEY, data TEXT)",
    )
    .unwrap();
    exec(
        &mut vm,
        "CREATE TABLE pk_b (id INTEGER PRIMARY KEY, data TEXT)",
    )
    .unwrap();
    ...
    for i in 0..30 {
        exec(
            &mut vm,
            &format!("INSERT INTO pk_a VALUES ({}, 'a{}')", i, i),
        )
        .unwrap();
        exec(
            &mut vm,
            &format!("INSERT INTO pk_b VALUES ({}, 'b{}')", i, i),
        )
        .unwrap();
    }
...
fn test_join_reorder_with_where() {
    ...
    exec(
        &mut vm,
        "CREATE TABLE wr_a (id INTEGER PRIMARY KEY, v INTEGER)",
    )
    .unwrap();
    exec(
        &mut vm,
        "CREATE TABLE wr_b (id INTEGER PRIMARY KEY, v INTEGER)",
    )
    .unwrap();
    ...
    for i in 0..100 {
        exec(
            &mut vm,
            &format!("INSERT INTO wr_a VALUES ({}, {})", i, i % 10),
        )
        .unwrap();
    }
    for i in 0..20 {
        exec(
            &mut vm,
            &format!("INSERT INTO wr_b VALUES ({}, {})", i, i % 5),
        )
        .unwrap();
    }
...
fn test_pager_wal_checkpoint_method() {
    ...
    let _ = applied; // exercises the path
}
...
fn test_wal_group_commit_multiple_auto_syncs() {
    ...
    assert!(
        stats.total_fsyncs >= 3,
        "expected ≥3 fsyncs, got {}",
        stats.total_fsyncs
    );
}
...
fn test_cbo_cost_model_with_multi_column_join() {
    ...
    exec(
        &mut vm,
        "CREATE TABLE mc_a (id INTEGER PRIMARY KEY, x INTEGER, y INTEGER)",
    )
    .unwrap();
    exec(
        &mut vm,
        "CREATE TABLE mc_b (id INTEGER PRIMARY KEY, x INTEGER, y INTEGER)",
    )
    .unwrap();
    ...
    for i in 0..50 {
        exec(
            &mut vm,
            &format!("INSERT INTO mc_a VALUES ({}, {}, {})", i, i % 10, i % 5),
        )
        .unwrap();
        exec(
            &mut vm,
            &format!("INSERT INTO mc_b VALUES ({}, {}, {})", i, i % 5, i % 10),
        )
        .unwrap();
    }
