...
fn test_right_join_all_unmatched_left() {
    ...
    vm.execute_sql("CREATE TABLE rj_l (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE rj_r (id INTEGER PRIMARY KEY, ref_id INTEGER, w TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO rj_l VALUES (1,'a'),(2,'b')")
        .unwrap();
    vm.execute_sql("INSERT INTO rj_r VALUES (1,10,'x'),(2,20,'y')")
        .unwrap();
...
fn test_right_join_partial_match() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rjp_l (id INTEGER PRIMARY KEY, k INTEGER, v TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE rjp_r (id INTEGER PRIMARY KEY, k INTEGER, w TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO rjp_l VALUES (1,100,'a'),(2,200,'b')")
        .unwrap();
    vm.execute_sql("INSERT INTO rjp_r VALUES (1,100,'x'),(2,300,'y'),(3,400,'z')")
        .unwrap();
...
fn test_right_join_with_complex_on() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rjc_a (id INTEGER PRIMARY KEY, x INTEGER)")
        .unwrap();
    vm.execute_sql("CREATE TABLE rjc_b (id INTEGER PRIMARY KEY, y INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO rjc_a VALUES (1,10),(2,20),(3,30)")
        .unwrap();
    vm.execute_sql("INSERT INTO rjc_b VALUES (1,10),(2,15),(3,30)")
        .unwrap();
    let rows = query_rows(&mut vm,
...
fn test_full_join_three_way_overlap() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fj_l (id INTEGER PRIMARY KEY, k INTEGER, lv TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE fj_r (id INTEGER PRIMARY KEY, k INTEGER, rv TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO fj_l VALUES (1,1,'L1'),(2,2,'L2'),(3,3,'L3')")
        .unwrap();
    vm.execute_sql("INSERT INTO fj_r VALUES (1,2,'R2'),(2,4,'R4')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_full_join_empty_tables() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fje_l (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE fje_r (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_full_join_one_empty() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fjo_l (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE fjo_r (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO fjo_l VALUES (1,'a'),(2,'b')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_left_semi_join_via_exists_large() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE lsj_a (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("CREATE TABLE lsj_b (id INTEGER PRIMARY KEY, ref_v INTEGER)")
        .unwrap();
    for i in 0..50 {
        vm.execute_sql(&format!("INSERT INTO lsj_a VALUES ({}, {})", i, i * 10))
            .unwrap();
    }
    for i in 0..20 {
        vm.execute_sql(&format!("INSERT INTO lsj_b VALUES ({}, {})", i, i * 10))
            .unwrap();
    }
    let rows = query_rows(&mut vm,
...
fn test_semi_join_in_subquery() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE sj1 (id INTEGER PRIMARY KEY, code TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE sj2 (id INTEGER PRIMARY KEY, code TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO sj1 VALUES (1,'A'),(2,'B'),(3,'C'),(4,'D')")
        .unwrap();
    vm.execute_sql("INSERT INTO sj2 VALUES (1,'B'),(2,'D')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_having_with_like() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE hv_l (id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)")
        .unwrap();
    vm.execute_sql(
        "INSERT INTO hv_l VALUES (1,'alpha',10),(2,'alpha',20),(3,'beta',30),(4,'gamma',40)",
    )
    .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_having_with_ilike() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE hv_i (id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO hv_i VALUES (1,'Alpha',10),(2,'BETA',20),(3,'alpha',30)")
        .unwrap();
...
fn test_before_insert_trigger() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE trig_t (id INTEGER PRIMARY KEY, val INTEGER, log TEXT)")
        .unwrap();
    ...
    if res.is_ok() {
        vm.execute_sql("INSERT INTO trig_t VALUES (1, 100, 'test')")
            .unwrap();
...
fn test_after_update_trigger() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE trig_u (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql(
        "CREATE TABLE trig_u_log (id INTEGER PRIMARY KEY, old_val INTEGER, new_val INTEGER)",
    )
    .unwrap();
    let res = vm.execute_sql(
    ...
    if res.is_ok() {
        ...
        vm.execute_sql("UPDATE trig_u SET val = 20 WHERE id = 1")
            .unwrap();
...
fn test_after_delete_trigger() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE trig_d (id INTEGER PRIMARY KEY, name TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE trig_d_log (id INTEGER PRIMARY KEY, deleted_name TEXT)")
        .unwrap();
    let res = vm.execute_sql(
    ...
    if res.is_ok() {
        vm.execute_sql("INSERT INTO trig_d VALUES (1, 'alice')")
            .unwrap();
...
fn test_foreign_key_schema_restore() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fk_par (id INTEGER PRIMARY KEY, name TEXT)")
        .unwrap();
    vm.execute_sql(
        "CREATE TABLE fk_child (id INTEGER PRIMARY KEY, par_id INTEGER REFERENCES fk_par(id))",
    )
    .unwrap();
    vm.execute_sql("INSERT INTO fk_par VALUES (1, 'parent')")
        .unwrap();
    vm.execute_sql("INSERT INTO fk_child VALUES (1, 1)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT p.name FROM fk_par p JOIN fk_child c ON c.par_id = p.id",
    );
    assert_eq!(rows.len(), 1);
...
fn test_foreign_key_with_actions() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fka_par (id INTEGER PRIMARY KEY)")
        .unwrap();
    let res = vm.execute_sql(
    ...
    vm.execute_sql("INSERT INTO fka_child VALUES (1, 1)")
        .unwrap();
...
fn test_check_constraint_column_level() {
    ...
    vm.execute_sql("INSERT INTO chk_col VALUES (1, 25)")
        .unwrap();
...
fn test_check_constraint_table_level() {
    ...
    vm.execute_sql("INSERT INTO chk_tbl VALUES (1, 10, 20)")
        .unwrap();
...
fn test_large_blob_overflow_chain() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE blob_large (id INTEGER PRIMARY KEY, data TEXT)")
        .unwrap();
    // Insert a very large text value to trigger overflow pages
    let big = "A".repeat(16000);
    vm.execute_sql(&format!("INSERT INTO blob_large VALUES (1, '{}')", big))
        .unwrap();
...
fn test_multiple_overflow_rows() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE blob_multi (id INTEGER PRIMARY KEY, data TEXT)")
        .unwrap();
    for i in 0..5 {
        let big = format!("{}_{}", "B".repeat(10000), i);
        vm.execute_sql(&format!("INSERT INTO blob_multi VALUES ({}, '{}')", i, big))
            .unwrap();
    }
    ...
    for i in 0..5 {
        let rows = query_rows(
            &mut vm,
            &format!("SELECT LENGTH(data) FROM blob_multi WHERE id = {}", i),
        );
        match &rows[0][0] {
...
fn test_subquery_from_union() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE sf1 (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("CREATE TABLE sf2 (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO sf1 VALUES (1,10),(2,20)")
        .unwrap();
    vm.execute_sql("INSERT INTO sf2 VALUES (1,30),(2,40)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_auto_txn_insert_success() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE at1 (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    // No explicit BEGIN — auto-transaction
    vm.execute_sql("INSERT INTO at1 VALUES (1, 'auto_txn')")
        .unwrap();
...
fn test_auto_txn_insert_violation_rollback() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE at2 (id INTEGER PRIMARY KEY, v TEXT NOT NULL)")
        .unwrap();
...
fn test_index_scan_with_multiple_conditions() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE idx_mc (id INTEGER PRIMARY KEY, a TEXT, b INTEGER, c TEXT)")
        .unwrap();
    vm.execute_sql("CREATE INDEX idx_mc_a ON idx_mc (a)")
        .unwrap();
    vm.execute_sql("CREATE INDEX idx_mc_b ON idx_mc (b)")
        .unwrap();
    for i in 0..100 {
    ...
    let rows = query_rows(
        &mut vm,
        "SELECT COUNT(*) FROM idx_mc WHERE a = 'x' AND b < 100",
    );
...
fn test_view_with_aggregation() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE vw_src (id INTEGER PRIMARY KEY, cat TEXT, amt INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO vw_src VALUES (1,'a',10),(2,'a',20),(3,'b',30)")
        .unwrap();
    vm.execute_sql("CREATE VIEW vw_agg AS SELECT cat, SUM(amt) as total FROM vw_src GROUP BY cat")
        .unwrap();
...
fn test_view_nested() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE vn_src (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO vn_src VALUES (1,10),(2,20),(3,30)")
        .unwrap();
    vm.execute_sql("CREATE VIEW vn_v1 AS SELECT id, v * 2 as doubled FROM vn_src")
        .unwrap();
    vm.execute_sql("CREATE VIEW vn_v2 AS SELECT id, doubled FROM vn_v1 WHERE doubled > 25")
        .unwrap();
...
fn test_case_insensitive_table_name() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE MyTable (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO MyTable VALUES (1, 'hello')")
        .unwrap();
...
fn test_case_insensitive_column_name() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ci_col (ID INTEGER PRIMARY KEY, MyValue TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO ci_col VALUES (1, 'test')")
        .unwrap();
...
fn test_window_over_grouped_query() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE wg (id INTEGER PRIMARY KEY, grp TEXT, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO wg VALUES (1,'a',10),(2,'a',20),(3,'b',30),(4,'b',40),(5,'c',50)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_rank_over_grouped() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rg (id INTEGER PRIMARY KEY, cat TEXT, score INTEGER)")
        .unwrap();
    vm.execute_sql(
        "INSERT INTO rg VALUES (1,'a',100),(2,'a',200),(3,'b',150),(4,'b',150),(5,'c',300)",
    )
    .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_union_all_aggregation() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ua1 (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("CREATE TABLE ua2 (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO ua1 VALUES (1,10),(2,20)")
        .unwrap();
    vm.execute_sql("INSERT INTO ua2 VALUES (1,30),(2,40)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_autoincrement_insert() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ai (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO ai (name) VALUES ('first')")
        .unwrap();
    vm.execute_sql("INSERT INTO ai (name) VALUES ('second')")
        .unwrap();
    vm.execute_sql("INSERT INTO ai (name) VALUES ('third')")
        .unwrap();
...
fn test_nested_case_in_select() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nc (id INTEGER PRIMARY KEY, a INTEGER, b INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO nc VALUES (1,10,20),(2,30,5),(3,NULL,15),(4,0,0)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT id, CASE \
...
fn test_recursive_cte_tree() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE tree (id INTEGER PRIMARY KEY, parent_id INTEGER, name TEXT)")
        .unwrap();
...
fn test_distinct_multi_column() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE dm (id INTEGER PRIMARY KEY, a TEXT, b INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO dm VALUES (1,'x',1),(2,'x',1),(3,'x',2),(4,'y',1),(5,'y',1)")
        .unwrap();
...
fn test_fulltext_index_create_insert_search() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ft (id INTEGER PRIMARY KEY, title TEXT, body TEXT)")
        .unwrap();
    let res = vm.execute_sql("CREATE FULLTEXT INDEX ft_idx ON ft (title, body)");
    if res.is_ok() {
        vm.execute_sql("INSERT INTO ft VALUES (1, 'rust programming', 'learn rust basics')")
            .unwrap();
        vm.execute_sql("INSERT INTO ft VALUES (2, 'python guide', 'python for beginners')")
            .unwrap();
        let rows = query_rows(
            &mut vm,
...
fn test_btree_1000_rows_with_scan() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE big1k (id INTEGER PRIMARY KEY, a INTEGER, b TEXT)")
        .unwrap();
    for i in 0..1000 {
        vm.execute_sql(&format!(
...
fn test_btree_delete_half_and_scan() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE dh (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    for i in 0..500 {
        vm.execute_sql(&format!("INSERT INTO dh VALUES ({}, {})", i, i))
            .unwrap();
    }
    // Delete odd rows
