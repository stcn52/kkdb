...
fn setup_ab() -> VM {
    ...
    vm.execute_sql("CREATE TABLE a (id INTEGER PRIMARY KEY, x TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE b (id INTEGER PRIMARY KEY, y TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO a VALUES (1,'a1'),(2,'a2'),(3,'a3')")
        .unwrap();
    vm.execute_sql("INSERT INTO b VALUES (2,'b2'),(3,'b3'),(4,'b4')")
        .unwrap();
    vm
}
...
fn test_full_outer_join_empty_left() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE l (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE r (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO r VALUES (1,'r1'),(2,'r2')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT l.id, r.id FROM l FULL OUTER JOIN r ON l.id = r.id ORDER BY r.id",
    );
...
fn test_full_outer_join_all_match() {
    ...
    let rows = query_rows(
        &mut vm,
        "SELECT t1.id, t2.id FROM t1 FULL OUTER JOIN t2 ON t1.id = t2.id ORDER BY t1.id",
    );
    assert_eq!(rows.len(), 2);
    assert_eq!(rows[0][0], Value::Integer(1));
...
fn test_natural_join_basic() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nt1 (id INTEGER PRIMARY KEY, name TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE nt2 (id INTEGER PRIMARY KEY, score INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO nt1 VALUES (1,'Alice'),(2,'Bob')")
        .unwrap();
    vm.execute_sql("INSERT INTO nt2 VALUES (1,90),(2,85)")
        .unwrap();
...
fn test_natural_join_no_common_columns() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nx (a INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("CREATE TABLE ny (b INTEGER PRIMARY KEY)")
        .unwrap();
...
fn test_insert_returning() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rt (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    let res = vm
        .execute_sql("INSERT INTO rt VALUES (1,'hello') RETURNING *")
        .unwrap();
...
fn test_insert_returning_specific_cols() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rt2 (id INTEGER PRIMARY KEY, a TEXT, b TEXT)")
        .unwrap();
    let res = vm
        .execute_sql("INSERT INTO rt2 VALUES (1,'x','y') RETURNING id, a")
        .unwrap();
    if let ExecResult::QueryResult { rows, .. } = res {
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0][0], Value::Integer(1));
...
fn test_update_returning() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rt3 (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO rt3 VALUES (1,10),(2,20)")
        .unwrap();
    let res = vm
        .execute_sql("UPDATE rt3 SET val = val + 100 WHERE id = 1 RETURNING *")
        .unwrap();
    if let ExecResult::QueryResult { rows, .. } = res {
        assert_eq!(rows.len(), 1);
...
fn test_delete_returning() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rt4 (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO rt4 VALUES (1,'a'),(2,'b'),(3,'c')")
        .unwrap();
    let res = vm
        .execute_sql("DELETE FROM rt4 WHERE id = 2 RETURNING *")
        .unwrap();
    if let ExecResult::QueryResult { rows, .. } = res {
        assert_eq!(rows.len(), 1);
...
fn test_insert_or_replace_new() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE kv (key TEXT PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT OR REPLACE INTO kv VALUES ('a', 1)")
        .unwrap();
...
fn test_insert_or_replace_existing() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE kv2 (key TEXT PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO kv2 VALUES ('a', 1)").unwrap();
    vm.execute_sql("INSERT OR REPLACE INTO kv2 VALUES ('a', 200)")
        .unwrap();
...
fn test_insert_or_replace_multiple_rows() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE kv3 (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO kv3 VALUES (1, 10)").unwrap();
    vm.execute_sql("INSERT OR REPLACE INTO kv3 VALUES (1, 99),(2, 20)")
        .unwrap();
...
fn test_window_sum_over_partition() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE wsales (id INTEGER PRIMARY KEY, dept TEXT, amt INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO wsales VALUES (1,'A',100),(2,'A',200),(3,'B',150),(4,'B',250)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_window_count_over() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE wc (id INTEGER PRIMARY KEY, dept TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO wc VALUES (1,'A'),(2,'A'),(3,'B'),(4,'B'),(5,'B')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_window_avg_over() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE wavg (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO wavg VALUES (1,10),(2,20),(3,30)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_window_max_min_over() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE wmm (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO wmm VALUES (1,30),(2,10),(3,20)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_percent_rank() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE pr (id INTEGER PRIMARY KEY, score INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO pr VALUES (1,90),(2,80),(3,70),(4,85)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_cume_dist() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE cd (id INTEGER PRIMARY KEY, score INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO cd VALUES (1,90),(2,80),(3,70),(4,85)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_nth_value() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nv (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO nv VALUES (1,'a'),(2,'b'),(3,'c')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_nth_value_out_of_range() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nv2 (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO nv2 VALUES (1,'a'),(2,'b')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_order_by_nulls_first() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nf (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO nf VALUES (1,NULL),(2,10),(3,5),(4,NULL)")
        .unwrap();
...
fn test_order_by_desc_nulls_last() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nl (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO nl VALUES (1,NULL),(2,10),(3,5),(4,NULL)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT id, val FROM nl ORDER BY val DESC NULLS LAST",
    );
    assert_eq!(rows.len(), 4);
...
fn test_intersect_all() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ia1 (x INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("CREATE TABLE ia2 (x INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("INSERT INTO ia1 VALUES (1),(2),(3)")
        .unwrap();
    vm.execute_sql("INSERT INTO ia2 VALUES (2),(3),(4)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT x FROM ia1 INTERSECT ALL SELECT x FROM ia2 ORDER BY x",
    );
    // Intersection: 2, 3
    assert_eq!(rows.len(), 2);
    assert_eq!(rows[0][0], Value::Integer(2));
...
fn test_except_all() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ea1 (x INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("CREATE TABLE ea2 (x INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("INSERT INTO ea1 VALUES (1),(2),(3)")
        .unwrap();
    vm.execute_sql("INSERT INTO ea2 VALUES (2),(4)").unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT x FROM ea1 EXCEPT ALL SELECT x FROM ea2 ORDER BY x",
    );
    // 1, 3 (removed 2)
    assert_eq!(rows.len(), 2);
    assert_eq!(rows[0][0], Value::Integer(1));
...
fn test_savepoint_release() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE sp2 (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("BEGIN").unwrap();
...
fn test_show_tables() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE alpha (id INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("CREATE TABLE beta (id INTEGER PRIMARY KEY)")
        .unwrap();
...
fn test_vacuum() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE vac (id INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("INSERT INTO vac VALUES (1),(2),(3)")
        .unwrap();
...
fn test_analyze_table() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE an (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    for i in 1..=100 {
        vm.execute_sql(&format!("INSERT INTO an VALUES ({i}, {v})", v = i % 10))
            .unwrap();
    }
...
fn test_fk_on_update_cascade() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fk_parent (id INTEGER PRIMARY KEY, name TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE fk_child (id INTEGER PRIMARY KEY, pid INTEGER REFERENCES fk_parent(id) ON UPDATE CASCADE)").unwrap();
    vm.execute_sql("INSERT INTO fk_parent VALUES (1,'Alice')")
        .unwrap();
    vm.execute_sql("INSERT INTO fk_child VALUES (1, 1)")
        .unwrap();
    vm.execute_sql("UPDATE fk_parent SET id = 10 WHERE id = 1")
        .unwrap();
...
fn test_fk_on_delete_cascade() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fk_p3 (id INTEGER PRIMARY KEY)")
        .unwrap();
    ...
    vm.execute_sql("INSERT INTO fk_c3 VALUES (1, 1),(2, 1),(3, 2)")
        .unwrap();
...
fn test_window_rows_between_preceding_following() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE wf (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO wf VALUES (1,10),(2,20),(3,30),(4,40),(5,50)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_window_rows_unbounded_preceding() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE wf2 (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO wf2 VALUES (1,10),(2,20),(3,30)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_alter_table_add_column() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE alt (id INTEGER PRIMARY KEY, a TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO alt VALUES (1,'x')").unwrap();
    vm.execute_sql("ALTER TABLE alt ADD COLUMN b INTEGER")
        .unwrap();
...
fn test_alter_table_drop_column() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE altd (id INTEGER PRIMARY KEY, a TEXT, b TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO altd VALUES (1,'x','y')")
        .unwrap();
...
fn test_alter_table_rename() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE old_name (id INTEGER PRIMARY KEY)")
        .unwrap();
...
fn test_insert_or_ignore() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ig (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO ig VALUES (1, 'a')").unwrap();
    vm.execute_sql("INSERT OR IGNORE INTO ig VALUES (1, 'b')")
        .unwrap();
...
fn test_insert_or_replace() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rp (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO rp VALUES (1, 'a')").unwrap();
    vm.execute_sql("INSERT OR REPLACE INTO rp VALUES (1, 'b')")
        .unwrap();
...
fn test_select_distinct() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE sd (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO sd VALUES (1,10),(2,10),(3,20),(4,20),(5,10)")
        .unwrap();
...
fn test_group_by_having() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE gh (id INTEGER PRIMARY KEY, grp TEXT, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO gh VALUES (1,'A',10),(2,'A',20),(3,'B',5),(4,'B',15),(5,'C',100)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT grp, SUM(val) AS s FROM gh GROUP BY grp HAVING SUM(val) > 25 ORDER BY grp",
    );
...
fn test_limit_offset() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE lo (id INTEGER PRIMARY KEY)")
        .unwrap();
    for i in 1..=10 {
        vm.execute_sql(&format!("INSERT INTO lo VALUES ({})", i))
            .unwrap();
    }
...
fn test_cross_join() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE cj1 (a INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("CREATE TABLE cj2 (b INTEGER PRIMARY KEY)")
        .unwrap();
...
fn test_exists_subquery() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE eo (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE ei (id INTEGER PRIMARY KEY, oid INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO eo VALUES (1,'a'),(2,'b'),(3,'c')")
        .unwrap();
    vm.execute_sql("INSERT INTO ei VALUES (1,1),(2,2)").unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT val FROM eo WHERE EXISTS (SELECT 1 FROM ei WHERE ei.oid = eo.id) ORDER BY id",
    );
    assert_eq!(rows.len(), 2);
    assert_eq!(rows[0][0], Value::Text("a".into()));
...
fn test_in_subquery() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE io (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO io VALUES (1,'a'),(2,'b'),(3,'c')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT val FROM io WHERE id IN (SELECT id FROM io WHERE id < 3) ORDER BY id",
    );
    assert_eq!(rows.len(), 2);
}
...
fn test_case_when_expression() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE cw (id INTEGER PRIMARY KEY, grade INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO cw VALUES (1,90),(2,75),(3,60),(4,45)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_multiple_order_by_columns() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE mob (id INTEGER PRIMARY KEY, a INTEGER, b INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO mob VALUES (1,2,30),(2,1,20),(3,2,10),(4,1,40)")
        .unwrap();
...
fn test_union_distinct() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ud1 (x INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("CREATE TABLE ud2 (x INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("INSERT INTO ud1 VALUES (1),(2),(3)")
        .unwrap();
    vm.execute_sql("INSERT INTO ud2 VALUES (2),(3),(4)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_union_all() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ua1 (x INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("CREATE TABLE ua2 (x INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("INSERT INTO ua1 VALUES (1),(2)").unwrap();
    vm.execute_sql("INSERT INTO ua2 VALUES (2),(3)").unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT x FROM ua1 UNION ALL SELECT x FROM ua2 ORDER BY x",
    );
...
fn test_coalesce_multiple_args() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE co (id INTEGER PRIMARY KEY, a INTEGER, b INTEGER, c INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO co VALUES (1,NULL,NULL,30),(2,NULL,20,NULL),(3,10,NULL,NULL)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT id, COALESCE(a, b, c) AS first_non_null FROM co ORDER BY id",
    );
...
fn test_is_null_is_not_null() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE isnl (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO isnl VALUES (1,NULL),(2,10),(3,NULL),(4,20)")
        .unwrap();
...
fn test_between() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE bet (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO bet VALUES (1,5),(2,10),(3,15),(4,20),(5,25)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT id FROM bet WHERE v BETWEEN 10 AND 20 ORDER BY id",
    );
...
fn test_like_with_escape() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE lk (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO lk VALUES (1,'hello'),(2,'world'),(3,'he%llo')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT val FROM lk WHERE val LIKE 'he%' ORDER BY id",
    );
...
fn test_nested_subquery_in_from() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nsq (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO nsq VALUES (1,10),(2,20),(3,30)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
