...
fn test_vec_normalize_text_input() {
    ...
    let rows = query_rows(
        &mut vm,
        "SELECT VEC_DIM(VEC_NORMALIZE(VEC('[1.0, 0.0, 0.0]')))",
    );
    assert_eq!(rows[0][0], Value::Integer(3));
}
...
fn test_vec_already_blob() {
    ...
    let mut vm = VM::new_memory();
    let rows = query_rows(&mut vm, "SELECT VEC_DIM(VEC(VEC('[1.0, 2.0, 3.0]')))");
    assert_eq!(rows[0][0], Value::Integer(3));
}
...
fn test_date_extract_year_from_epoch() {
    ...
    let rows = query_rows(&mut vm, "SELECT DATE_EXTRACT('YEAR', 1609459200)");
...
fn test_date_extract_month_from_epoch() {
    ...
    let rows = query_rows(&mut vm, "SELECT DATE_EXTRACT('MONTH', 1609459200)");
    assert_eq!(rows[0][0], Value::Integer(1));
}
...
fn test_date_extract_day_from_epoch() {
    let mut vm = VM::new_memory();
    let rows = query_rows(&mut vm, "SELECT DATE_EXTRACT('DAY', 1609459200)");
    assert_eq!(rows[0][0], Value::Integer(1));
}
...
fn test_date_extract_hour_from_epoch() {
    ...
    let rows = query_rows(&mut vm, "SELECT DATE_EXTRACT('HOUR', 1609502400)");
...
fn test_date_extract_minute_from_epoch() {
    ...
    let rows = query_rows(&mut vm, "SELECT DATE_EXTRACT('MINUTE', 1609459260)");
    assert_eq!(rows[0][0], Value::Integer(1));
}
...
fn test_date_extract_second_from_epoch() {
    ...
    let rows = query_rows(&mut vm, "SELECT DATE_EXTRACT('SECOND', 1609459245)");
...
fn test_date_extract_unknown_field() {
    let mut vm = VM::new_memory();
    let rows = query_rows(&mut vm, "SELECT DATE_EXTRACT('WEEK', 1609459200)");
    assert_eq!(rows[0][0], Value::Null);
}
...
fn test_interval_in_group_by_text() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE t_intv (id INTEGER PRIMARY KEY, grp TEXT, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO t_intv VALUES (1, 'a', 10), (2, 'a', 20), (3, 'b', 30)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT grp, SUM(val), INTERVAL '1' DAY FROM t_intv GROUP BY grp",
    );
...
fn test_interval_in_group_by_integer() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE t_intv2 (id INTEGER PRIMARY KEY, grp TEXT, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO t_intv2 VALUES (1, 'x', 5), (2, 'y', 10)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
        "SELECT grp, INTERVAL val MONTH FROM t_intv2 GROUP BY grp, val",
    );
    assert_eq!(rows.len(), 2);
}
...
fn test_interval_in_group_by_real() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE t_intv3 (id INTEGER PRIMARY KEY, v REAL)")
        .unwrap();
    vm.execute_sql("INSERT INTO t_intv3 VALUES (1, 1.5), (2, 2.5)")
        .unwrap();
...
fn test_interval_in_group_by_null() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE t_intv4 (id INTEGER PRIMARY KEY, v TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO t_intv4 VALUES (1, NULL)")
        .unwrap();
...
fn test_interval_no_leading_field_in_group() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE t_intv5 (id INTEGER PRIMARY KEY, v INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO t_intv5 VALUES (1, 42), (2, 42)")
        .unwrap();
...
fn test_collate_in_aggregate_context() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE t_coll (id INTEGER PRIMARY KEY, name TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO t_coll VALUES (1, 'Alice'), (2, 'Bob')")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_left_semi_join_equi() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ls_a (id INTEGER PRIMARY KEY, name TEXT)")
        .unwrap();
    vm.execute_sql("CREATE TABLE ls_b (id INTEGER PRIMARY KEY, ref_id INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO ls_a VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie')")
        .unwrap();
    vm.execute_sql("INSERT INTO ls_b VALUES (1, 1), (2, 3), (3, 3)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_left_semi_join_non_equi() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE lsn_a (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("CREATE TABLE lsn_b (id INTEGER PRIMARY KEY, threshold INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO lsn_a VALUES (1, 10), (2, 20), (3, 30)")
        .unwrap();
    vm.execute_sql("INSERT INTO lsn_b VALUES (1, 15), (2, 25)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_left_semi_join_no_condition() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE lsc_a (id INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("CREATE TABLE lsc_b (id INTEGER PRIMARY KEY)")
        .unwrap();
...
fn test_right_semi_join_equi() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rs_a (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("CREATE TABLE rs_b (id INTEGER PRIMARY KEY, ref_val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO rs_a VALUES (1, 100), (2, 200)")
        .unwrap();
    vm.execute_sql("INSERT INTO rs_b VALUES (1, 100), (2, 300), (3, 200)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_right_semi_join_non_equi() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE rsn_a (id INTEGER PRIMARY KEY, low INTEGER)")
        .unwrap();
    vm.execute_sql("CREATE TABLE rsn_b (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO rsn_a VALUES (1, 5)").unwrap();
    vm.execute_sql("INSERT INTO rsn_b VALUES (1, 10), (2, 3), (3, 7)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_insert_or_replace_conflict() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE oc_tbl (id INTEGER PRIMARY KEY, name TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO oc_tbl VALUES (1, 'original')")
        .unwrap();
    vm.execute_sql("INSERT OR REPLACE INTO oc_tbl VALUES (1, 'replaced')")
        .unwrap();
...
fn test_insert_or_ignore_conflict() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE oc_tbl2 (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO oc_tbl2 VALUES (1, 'first')")
        .unwrap();
    vm.execute_sql("INSERT OR IGNORE INTO oc_tbl2 VALUES (1, 'second')")
        .unwrap();
...
fn test_on_conflict_do_update_error() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE oc_tbl3 (id INTEGER PRIMARY KEY)")
        .unwrap();
    let result =
        vm.execute_sql("INSERT INTO oc_tbl3 VALUES (1) ON CONFLICT (id) DO UPDATE SET id = 2");
    assert!(result.is_err());
}
...
fn test_large_table_cursor_interior_traversal() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE big_t (id INTEGER PRIMARY KEY, data TEXT)")
        .unwrap();
...
fn test_large_table_ordered_scan() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE big_t2 (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
...
fn test_large_table_delete_and_scan() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE big_t3 (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    ...
    vm.execute_sql("DELETE FROM big_t3 WHERE id % 5 = 0")
        .unwrap();
...
fn test_drop_vector_index_success() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE vec_tbl (id INTEGER PRIMARY KEY, emb BLOB)")
        .unwrap();
    // Create a vector index
    let r =
        vm.execute_sql("CREATE VECTOR INDEX vec_idx ON vec_tbl(emb) DIMENSION 4 DISTANCE cosine");
    if r.is_ok() {
        // Drop it
...
fn test_interval_scalar_text() {
    ...
    if let Value::Text(s) = &rows[0][0] {
        assert!(
            s.contains("MINUTE") || s.contains("30"),
            "got: {}",
            s.as_ref()
        );
    }
...
fn test_interval_scalar_integer() {
    ...
    if let Value::Text(s) = &rows[0][0] {
        assert!(
            s.contains("5") || s.contains("SECOND"),
            "got: {}",
            s.as_ref()
        );
    }
...
fn test_vec_search_no_index() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE vs_t (id INTEGER PRIMARY KEY, emb BLOB)")
        .unwrap();
...
fn test_percent_rank_with_order_by_group() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE pr_tbl (id INTEGER PRIMARY KEY, grp TEXT, score INTEGER)")
        .unwrap();
    vm.execute_sql(
        "INSERT INTO pr_tbl VALUES (1,'a',10),(2,'a',20),(3,'a',30),(4,'b',40),(5,'b',50)",
    )
    .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_percent_rank_with_order_by_group_ties() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE pr_ties (id INTEGER PRIMARY KEY, grp TEXT, score INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO pr_ties VALUES (1,'x',10),(2,'x',10),(3,'x',20),(4,'x',30)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_cume_dist_with_order_by_group() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE cd_tbl (id INTEGER PRIMARY KEY, grp TEXT, score INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO cd_tbl VALUES (1,'a',10),(2,'a',20),(3,'a',30),(4,'b',40)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_cume_dist_with_ties() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE cd_ties (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO cd_ties VALUES (1,10),(2,10),(3,20),(4,30)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_nth_value_basic() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nv_tbl (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO nv_tbl VALUES (1,100),(2,200),(3,300)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_nth_value_out_of_range() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE nv2 (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO nv2 VALUES (1,10),(2,20)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_window_frame_preceding_end() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE wf_tbl (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO wf_tbl VALUES (1,10),(2,20),(3,30),(4,40),(5,50)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_window_frame_following_end() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE wf2 (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO wf2 VALUES (1,10),(2,20),(3,30),(4,40)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_top_n_with_offset() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE topn (id INTEGER PRIMARY KEY, val TEXT)")
        .unwrap();
    for i in 1..=20 {
        vm.execute_sql(&format!("INSERT INTO topn VALUES ({}, 'row_{:02}')", i, i))
            .unwrap();
    }
    let rows = query_rows(&mut vm, "SELECT id FROM topn ORDER BY id LIMIT 5 OFFSET 3");
    assert_eq!(rows.len(), 5);
...
fn test_top_n_select_nth() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE topn2 (id INTEGER PRIMARY KEY, val INTEGER)")
        .unwrap();
    for i in 1..=50 {
        vm.execute_sql(&format!("INSERT INTO topn2 VALUES ({}, {})", i, i * 10))
            .unwrap();
    }
    // ORDER BY + LIMIT where k < n triggers select_nth_unstable_by
    let rows = query_rows(&mut vm, "SELECT val FROM topn2 ORDER BY id LIMIT 5");
    assert_eq!(rows.len(), 5);
...
fn test_dense_rank_with_order() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE dr_tbl (id INTEGER PRIMARY KEY, grp TEXT, score INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO dr_tbl VALUES (1,'a',10),(2,'a',10),(3,'a',20),(4,'b',30)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_table_star_in_group_by() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ts_tbl (id INTEGER PRIMARY KEY, grp TEXT, val INTEGER)")
        .unwrap();
    vm.execute_sql("INSERT INTO ts_tbl VALUES (1,'a',10),(2,'b',20),(3,'a',30)")
        .unwrap();
    let rows = query_rows(
        &mut vm,
...
fn test_match_against_columns_fts() {
    ...
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fts_ma (id INTEGER PRIMARY KEY, title TEXT, body TEXT)")
        .unwrap();
    vm.execute_sql("CREATE FULLTEXT INDEX fts_ma_idx ON fts_ma(title, body)")
        .unwrap();
    vm.execute_sql("INSERT INTO fts_ma VALUES (1, 'hello world', 'this is a test')")
        .unwrap();
    vm.execute_sql("INSERT INTO fts_ma VALUES (2, 'foo bar', 'another row here')")
        .unwrap();
...
fn test_match_against_all_fts() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fts_ma2 (id INTEGER PRIMARY KEY, content TEXT)")
        .unwrap();
    vm.execute_sql("CREATE FULLTEXT INDEX fts_ma2_idx ON fts_ma2(content)")
        .unwrap();
    vm.execute_sql("INSERT INTO fts_ma2 VALUES (1, 'hello world')")
        .unwrap();
...
fn test_fts_match_scoring_multiple_tokens() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fts_score (id INTEGER PRIMARY KEY, content TEXT)")
        .unwrap();
    vm.execute_sql("CREATE FULLTEXT INDEX fts_score_idx ON fts_score(content)")
        .unwrap();
    vm.execute_sql("INSERT INTO fts_score VALUES (1, 'the quick brown fox jumps')")
        .unwrap();
    vm.execute_sql("INSERT INTO fts_score VALUES (2, 'lazy dog sleeps')")
        .unwrap();
    let result =
        vm.execute_sql("SELECT * FROM fts_score WHERE MATCH(content) AGAINST ('quick fox')");
    let _ = result;
}
...
fn test_json_member_of_text() {
    ...
    let rows = query_rows(
        &mut vm,
        r#"SELECT JSON_MEMBER_OF('hello', '["hello","world"]')"#,
    );
    assert_eq!(rows[0][0], Value::Integer(1));
}
...
fn test_match_against_eval_expr_path() {
    ...
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ma_test (id INTEGER PRIMARY KEY, title TEXT, body TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO ma_test VALUES (1, 'hello world', 'test')")
        .unwrap();
    vm.execute_sql("INSERT INTO ma_test VALUES (2, 'foo bar', 'other')")
        .unwrap();
    vm.execute_sql("INSERT INTO ma_test VALUES (3, 'hello again', 'more text')")
        .unwrap();
...
fn test_match_against_eval_expr_columns_path() {
    ...
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ma_col (id INTEGER PRIMARY KEY, name TEXT, descr TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO ma_col VALUES (1, 'data science', 'machine learning course')")
        .unwrap();
    vm.execute_sql("INSERT INTO ma_col VALUES (2, 'web dev', 'frontend development')")
        .unwrap();
...
fn test_match_against_eval_no_match() {
    ...
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ma_no (id INTEGER PRIMARY KEY, content TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO ma_no VALUES (1, 'hello world')")
        .unwrap();
    let result =
...
fn test_match_against_eval_empty_query() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ma_emp (id INTEGER PRIMARY KEY, content TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO ma_emp VALUES (1, 'test')")
        .unwrap();
...
fn test_match_against_no_columns() {
    ...
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE ma_nc (id INTEGER PRIMARY KEY, data TEXT)")
        .unwrap();
    vm.execute_sql("INSERT INTO ma_nc VALUES (1, 'some unique text')")
        .unwrap();
...
fn test_alter_drop_column_single_column_table() {
    ...
    vm.execute_sql("INSERT INTO single_col VALUES ('hello')")
        .unwrap();
...
fn test_alter_drop_pk_column_error() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE pk_tbl (id INTEGER PRIMARY KEY, name TEXT)")
        .unwrap();
    let result = vm.execute_sql("ALTER TABLE pk_tbl DROP COLUMN id");
    assert!(
        result.is_err(),
        "Should fail when dropping primary key column"
    );
    let err_msg = format!("{}", result.unwrap_err());
    assert!(err_msg.contains("PRIMARY KEY"), "Error: {}", err_msg);
}
...
fn test_create_fts_index_if_not_exists_duplicate() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fts_dup (id INTEGER PRIMARY KEY, body TEXT)")
        .unwrap();
    vm.execute_sql("CREATE FULLTEXT INDEX IF NOT EXISTS idx_fts_dup ON fts_dup(body)")
        .unwrap();
...
fn test_create_fts_index_duplicate_without_if_not_exists() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE fts_dup2 (id INTEGER PRIMARY KEY, content TEXT)")
        .unwrap();
    vm.execute_sql("CREATE FULLTEXT INDEX idx_fts_dup2 ON fts_dup2(content)")
        .unwrap();
    ...
    assert!(
        result.is_err(),
        "Duplicate FTS index without IF NOT EXISTS should fail"
    );
}
...
fn test_create_or_replace_trigger() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE trig_tbl (id INTEGER PRIMARY KEY, val INTEGER DEFAULT 0)")
        .unwrap();
    ...
    vm.execute_sql("INSERT INTO trig_tbl (id) VALUES (1)")
        .unwrap();
    ...
    if let Value::Text(s) = &rows[0][0] {
        assert_eq!(
            s.as_ref(),
            "v2",
            "Replaced trigger should fire with new body"
        );
    }
}
...
fn test_alter_add_column_default_negative_integer() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE def_neg (id INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("INSERT INTO def_neg VALUES (1)").unwrap();
    vm.execute_sql("ALTER TABLE def_neg ADD COLUMN score INTEGER DEFAULT -42")
        .unwrap();
...
fn test_alter_add_column_default_negative_real() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE def_negr (id INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("INSERT INTO def_negr VALUES (1)").unwrap();
    vm.execute_sql("ALTER TABLE def_negr ADD COLUMN weight REAL DEFAULT -3.14")
        .unwrap();
...
fn test_alter_add_column_default_blob_literal() {
    let mut vm = VM::new_memory();
    vm.execute_sql("CREATE TABLE def_blob (id INTEGER PRIMARY KEY)")
        .unwrap();
    vm.execute_sql("INSERT INTO def_blob VALUES (1)").unwrap();
    // X'CAFE' is a blob literal
    vm.execute_sql("ALTER TABLE def_blob ADD COLUMN data BLOB DEFAULT X'CAFE'")
        .unwrap();
