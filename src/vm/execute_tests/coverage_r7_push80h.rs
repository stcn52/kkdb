...
fn test_group_by_with_percent_rank() {
    ...
    exec(&mut vm, "CREATE TABLE gbpr(id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)");
    for i in 1..=20 {
        exec(&mut vm, &format!("INSERT INTO gbpr VALUES ({i}, '{}', {})", 
            if i <= 10 { "A" } else { "B" }, i * 10));
    }
...
fn test_group_by_with_cume_dist() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE gbcd(id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)");
    for i in 1..=30 {
        exec(&mut vm, &format!("INSERT INTO gbcd VALUES ({i}, '{}', {})", 
            ["X", "Y", "Z"][i % 3], i));
    }
...
fn test_group_by_with_dense_rank() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE gbdr(id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)");
    for i in 1..=20 {
        exec(&mut vm, &format!("INSERT INTO gbdr VALUES ({i}, '{}', {})", 
            ["A", "B", "C", "D"][i % 4], i));
    }
...
fn test_explain_subquery() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE esq(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "INSERT INTO esq VALUES (1, 10), (2, 20)");
    
    let r = try_exec(&mut vm, "EXPLAIN SELECT * FROM (SELECT * FROM esq WHERE val > 5) AS sub");
    assert!(r.is_ok());
}
...
fn test_large_insert_1000_rows() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE big(id INTEGER PRIMARY KEY, val TEXT, num INTEGER)");
    for i in 1..=1000 {
        exec(&mut vm, &format!("INSERT INTO big VALUES ({i}, 'item_{i}', {})", i * 7 % 100));
    }
    
    let rows = query_rows(&mut vm, "SELECT COUNT(*) FROM big");
    assert_eq!(rows[0][0], Value::Integer(1000));
    
    let rows = query_rows(&mut vm, "SELECT * FROM big WHERE num = 42 ORDER BY id LIMIT 5");
    assert!(rows.len() <= 5);
}
...
fn test_large_update_many_rows() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE big_upd(id INTEGER PRIMARY KEY, val INTEGER, status TEXT)");
    for i in 1..=500 {
        exec(&mut vm, &format!("INSERT INTO big_upd VALUES ({i}, {i}, 'pending')"));
    }
    
    exec(&mut vm, "UPDATE big_upd SET status = 'done' WHERE val <= 250");
    let rows = query_rows(&mut vm, "SELECT COUNT(*) FROM big_upd WHERE status = 'done'");
...
fn test_large_delete_many_rows() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE big_del(id INTEGER PRIMARY KEY, val INTEGER)");
    for i in 1..=500 {
...
fn test_aggregate_with_null_handling() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE anh(id INTEGER PRIMARY KEY, val REAL, cat TEXT)");
...
fn test_having_with_multiple_conditions() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE hmc(id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)");
    for i in 1..=30 {
        exec(&mut vm, &format!("INSERT INTO hmc VALUES ({i}, '{}', {})", 
            ["X", "Y", "Z"][i % 3], i));
    }
...
fn test_union_with_order_by_limit() {
    ...
    for i in 1..=10 { exec(&mut vm, &format!("INSERT INTO u1 VALUES ({i}, {i})")); }
    for i in 11..=20 { exec(&mut vm, &format!("INSERT INTO u2 VALUES ({i}, {i})")); }
...
fn test_union_with_offset() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE uo1(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE TABLE uo2(id INTEGER PRIMARY KEY, val INTEGER)");
    for i in 1..=5 { exec(&mut vm, &format!("INSERT INTO uo1 VALUES ({i}, {i})")); }
    for i in 6..=10 { exec(&mut vm, &format!("INSERT INTO uo2 VALUES ({i}, {i})")); }
...
fn test_null_safe_equality() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE nse(id INTEGER PRIMARY KEY, a INTEGER, b INTEGER)");
    exec(&mut vm, "INSERT INTO nse VALUES (1, 1, 1), (2, 1, 2), (3, NULL, NULL), (4, NULL, 1)");
    // IS comparison
...
fn test_in_subquery_with_empty_result() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE isqe1(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE TABLE isqe2(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "INSERT INTO isqe1 VALUES (1, 10), (2, 20)");
    // isqe2 is empty
    
    let rows = query_rows(&mut vm, "SELECT * FROM isqe1 WHERE val IN (SELECT val FROM isqe2)");
...
fn test_nested_case() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE nc2(id INTEGER PRIMARY KEY, a INTEGER, b TEXT)");
    exec(&mut vm, "INSERT INTO nc2 VALUES (1, 1, 'x'), (2, 2, 'y'), (3, 3, 'z')");
    let rows = query_rows(&mut vm,
...
fn test_group_by_with_ntile_window() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE gbn(id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)");
    for i in 1..=20 {
        exec(&mut vm, &format!("INSERT INTO gbn VALUES ({i}, '{}', {})", 
            ["A", "B", "C", "D"][i % 4], i));
    }
...
fn test_group_by_with_row_number_partition() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE gbrn(id INTEGER PRIMARY KEY, dept TEXT, role TEXT, sal INTEGER)");
...
fn test_view_creation_and_query() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE v_t(id INTEGER PRIMARY KEY, val TEXT, num INTEGER)");
    exec(&mut vm, "INSERT INTO v_t VALUES (1, 'a', 10), (2, 'b', 20), (3, 'c', 30)");
    exec(&mut vm, "CREATE VIEW v_high AS SELECT * FROM v_t WHERE num > 15");
...
fn test_view_with_aggregation() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE va_t(id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)");
    exec(&mut vm, "INSERT INTO va_t VALUES (1, 'A', 10), (2, 'A', 20), (3, 'B', 30)");
    exec(&mut vm, "CREATE VIEW va_summary AS SELECT cat, SUM(val) AS total FROM va_t GROUP BY cat");
...
fn test_index_comparison_operators() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ico(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_index_with_null_values() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE inull(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE INDEX idx_inull ON inull(val)");
    exec(&mut vm, "INSERT INTO inull VALUES (1, 10), (2, NULL), (3, 30), (4, NULL)");
...
fn test_multi_table_transaction_commit() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE mt_a(id INTEGER PRIMARY KEY, val TEXT)");
    exec(&mut vm, "CREATE TABLE mt_b(id INTEGER PRIMARY KEY, ref_id INTEGER, data TEXT)");
...
fn test_multi_table_transaction_rollback() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE mtr_a(id INTEGER PRIMARY KEY, val TEXT)");
    exec(&mut vm, "CREATE TABLE mtr_b(id INTEGER PRIMARY KEY, data TEXT)");
...
fn test_recursive_cte_tree_traversal() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE tree(id INTEGER PRIMARY KEY, name TEXT, parent_id INTEGER)");
...
fn test_schema_with_vector_index_restore() {
    ...
    
    {
        ...
        exec(&mut vm, "CREATE TABLE vec_t(id INTEGER PRIMARY KEY, data BLOB)");
        let r = try_exec(&mut vm, "CREATE VECTOR INDEX vec_idx ON vec_t(data) DIMENSION 3");
        let _ = r;
    }
...
fn test_schema_with_trigger_restore() {
    ...
    {
        let mut vm = VM::open(path).unwrap();
        exec(&mut vm, "CREATE TABLE trig_t(id INTEGER PRIMARY KEY, val INTEGER, log_val INTEGER)");
...
fn test_vm_file_with_wal_operations() {
    ...
    {
        let mut vm = VM::open(path).unwrap();
        exec(&mut vm, "CREATE TABLE wal_t(id INTEGER PRIMARY KEY, val TEXT)");
        for i in 1..=50 {
            exec(&mut vm, &format!("INSERT INTO wal_t VALUES ({i}, 'data_{i}')"));
        }
...
fn test_comparison_operators() {
    let mut vm = VM::new_memory();
    let rows = query_rows(&mut vm, "SELECT 1 < 2, 2 > 1, 1 <= 1, 1 >= 1, 1 = 1, 1 != 2");
