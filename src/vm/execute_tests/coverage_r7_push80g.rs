...
fn test_show_engine_status() {
    ...
    exec(&mut vm, "CREATE TABLE ses_t(id INTEGER PRIMARY KEY, val TEXT)");
    exec(&mut vm, "INSERT INTO ses_t VALUES (1, 'hello'), (2, 'world')");
...
fn test_show_engine_status_with_index() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE sei_t(id INTEGER PRIMARY KEY, val TEXT, num INTEGER)");
    exec(&mut vm, "CREATE INDEX idx_sei ON sei_t(num)");
    for i in 1..=20 {
        exec(&mut vm, &format!("INSERT INTO sei_t VALUES ({i}, 'row_{i}', {})", i % 5));
    }
...
fn test_order_by_limit_topn_optimization() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE topn(id INTEGER PRIMARY KEY, val INTEGER, label TEXT)");
    // Insert 100 rows so LIMIT << row count triggers top-N path
    for i in 1..=100 {
        exec(&mut vm, &format!("INSERT INTO topn VALUES ({i}, {}, 'item_{i}')", 100 - i));
    }
...
fn test_order_by_limit_with_offset() {
    ...
    let rows = query_rows(&mut vm, "SELECT * FROM topn2 ORDER BY val LIMIT 5 OFFSET 10");
...
fn test_order_by_desc_limit() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE topn3(id INTEGER PRIMARY KEY, val INTEGER)");
    for i in 1..=80 {
...
fn test_between_with_null_index() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE btw_idx(id INTEGER PRIMARY KEY, val INTEGER)");
    ...
    let rows = query_rows(&mut vm, "SELECT * FROM btw_idx WHERE val BETWEEN NULL AND 10");
    assert_eq!(rows.len(), 0);
}
...
fn test_between_reversed_range_index() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE btw_rev(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_between_normal_index() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE btw_norm(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_adaptive_auto_index_creation() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE auto_idx(id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)");
    for i in 1..=20 {
        exec(&mut vm, &format!("INSERT INTO auto_idx VALUES ({i}, 'cat_{ix}', {i})", ix = i % 3));
    }
...
fn test_adaptive_auto_index_skip_pk() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE auto_pk(id INTEGER PRIMARY KEY, val TEXT)");
    for i in 1..=10 {
        exec(&mut vm, &format!("INSERT INTO auto_pk VALUES ({i}, 'val_{i}')"));
    }
    ...
    let auto_idx_count = vm.schema.indexes.keys().filter(|k| k.contains("auto")).count();
    assert_eq!(auto_idx_count, 0, "should not create auto-index on PK column");
}

#[test]
fn test_adaptive_auto_index_skip_existing() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE auto_exist(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE INDEX idx_ae_val ON auto_exist(val)");
    for i in 1..=10 {
        exec(&mut vm, &format!("INSERT INTO auto_exist VALUES ({i}, {i})"));
    }
    ...
    let auto_idx_count = vm.schema.indexes.keys().filter(|k| k.contains("auto")).count();
    assert_eq!(auto_idx_count, 0, "should not create auto-index when index exists");
}
...
fn test_fts_match_query() {
    let mut vm = VM::new_memory();
    // Create FTS table
    exec(&mut vm, "CREATE VIRTUAL TABLE fts_test USING fts5(title, body)");
    exec(&mut vm, "INSERT INTO fts_test(title, body) VALUES ('hello world', 'this is a test body')");
    exec(&mut vm, "INSERT INTO fts_test(title, body) VALUES ('rust programming', 'systems programming language')");
    exec(&mut vm, "INSERT INTO fts_test(title, body) VALUES ('database engines', 'btree and pager')");
...
fn test_fts_match_no_results() {
    ...
    exec(&mut vm, "INSERT INTO fts_nr(content) VALUES ('hello world')");
    
    let r = try_exec(&mut vm, "SELECT * FROM fts_nr WHERE content MATCH 'nonexistent'");
    if let Ok(ExecResult::QueryResult { rows, .. }) = &r {
        assert_eq!(rows.len(), 0, "should find no results for non-matching term");
    }
}
...
fn test_table_star_with_window_function() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE tsw(id INTEGER PRIMARY KEY, val INTEGER, cat TEXT)");
    exec(&mut vm, "INSERT INTO tsw VALUES (1, 10, 'A'), (2, 20, 'B'), (3, 30, 'A')");
    ...
    if let Ok(ExecResult::QueryResult { rows, columns }) = &r {
        ...
        assert!(columns.len() >= 4, "should have id, val, cat, rn: got {:?}", columns);
    }
}
...
fn test_star_with_count_over() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE scwo(id INTEGER PRIMARY KEY, val TEXT)");
    exec(&mut vm, "INSERT INTO scwo VALUES (1, 'a'), (2, 'b'), (3, 'c')");
...
fn test_update_with_subquery() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE uws1(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE TABLE uws2(id INTEGER PRIMARY KEY, new_val INTEGER)");
    exec(&mut vm, "INSERT INTO uws1 VALUES (1, 0), (2, 0)");
    exec(&mut vm, "INSERT INTO uws2 VALUES (1, 100), (2, 200)");
    let r = try_exec(&mut vm, "UPDATE uws1 SET val = (SELECT new_val FROM uws2 WHERE uws2.id = uws1.id)");
    if r.is_ok() {
...
fn test_delete_with_subquery() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE dws(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "INSERT INTO dws VALUES (1, 10), (2, 20), (3, 30), (4, 40)");
    let r = try_exec(&mut vm, "DELETE FROM dws WHERE val > (SELECT AVG(val) FROM dws)");
    if r.is_ok() {
...
fn test_insert_in_explicit_transaction() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE txn_ins(id INTEGER PRIMARY KEY, val TEXT)");
    exec(&mut vm, "BEGIN");
...
fn test_update_in_explicit_transaction() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE txn_upd(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_create_table_if_not_exists() {
    ...
    exec(&mut vm, "CREATE TABLE IF NOT EXISTS cine(id INTEGER PRIMARY KEY)");
...
fn test_alter_table_add_column() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE alt_t(id INTEGER PRIMARY KEY, val TEXT)");
    exec(&mut vm, "INSERT INTO alt_t VALUES (1, 'hello')");
    let r = try_exec(&mut vm, "ALTER TABLE alt_t ADD COLUMN extra INTEGER DEFAULT 0");
    if r.is_ok() {
...
fn test_group_by_multiple_columns() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE gbm(id INTEGER PRIMARY KEY, cat TEXT, sub TEXT, val INTEGER)");
    for i in 1..=20 {
        exec(&mut vm, &format!("INSERT INTO gbm VALUES ({i}, '{}', '{}', {})",
            if i % 2 == 0 { "A" } else { "B" },
            if i % 3 == 0 { "X" } else { "Y" },
            i));
    }
...
fn test_aggregate_min_max() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE amm(id INTEGER PRIMARY KEY, val INTEGER)");
    for i in 1..=10 { exec(&mut vm, &format!("INSERT INTO amm VALUES ({i}, {})", i * 10)); }
...
fn test_window_percent_rank() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE wpr(id INTEGER PRIMARY KEY, val INTEGER)");
    for i in 1..=10 { exec(&mut vm, &format!("INSERT INTO wpr VALUES ({i}, {i})")); }
...
fn test_window_cume_dist() {
    ...
    for i in 1..=10 { exec(&mut vm, &format!("INSERT INTO wcd VALUES ({i}, {i})")); }
...
fn test_window_lag_lead() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE wll(id INTEGER PRIMARY KEY, val INTEGER)");
    for i in 1..=5 { exec(&mut vm, &format!("INSERT INTO wll VALUES ({i}, {})", i * 10)); }
...
fn test_window_first_last_value() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE wflv(id INTEGER PRIMARY KEY, val INTEGER, cat TEXT)");
    exec(&mut vm, "INSERT INTO wflv VALUES (1, 10, 'A'), (2, 20, 'A'), (3, 30, 'B'), (4, 40, 'B')");
...
fn test_order_by_multiple_columns() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE obm(id INTEGER PRIMARY KEY, a TEXT, b INTEGER)");
    exec(&mut vm, "INSERT INTO obm VALUES (1, 'A', 3), (2, 'B', 1), (3, 'A', 1), (4, 'B', 2)");
...
fn test_case_when_nested() {
    ...
    exec(&mut vm, "INSERT INTO cwn VALUES (1, 10), (2, 20), (3, NULL), (4, 0)");
    let rows = query_rows(&mut vm,
...
fn test_ifnull_function() {
    let mut vm = VM::new_memory();
    let rows = query_rows(&mut vm, "SELECT IFNULL(NULL, 'default'), IFNULL('value', 'default')");
...
fn test_string_functions_extended() {
    let mut vm = VM::new_memory();
    let rows = query_rows(&mut vm, "SELECT SUBSTR('hello world', 7, 5), INSTR('hello world', 'world')");
...
fn test_exists_subquery() {
    ...
    exec(&mut vm, "CREATE TABLE es2(id INTEGER PRIMARY KEY, es1_id INTEGER)");
...
fn test_not_exists_subquery() {
    ...
    exec(&mut vm, "CREATE TABLE ne2(id INTEGER PRIMARY KEY, ne1_id INTEGER)");
...
fn test_left_join_with_null() {
    ...
    exec(&mut vm, "CREATE TABLE lj2(id INTEGER PRIMARY KEY, lj1_id INTEGER, data TEXT)");
...
fn test_three_way_join() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE tw1(id INTEGER PRIMARY KEY, name TEXT)");
    exec(&mut vm, "CREATE TABLE tw2(id INTEGER PRIMARY KEY, tw1_id INTEGER, label TEXT)");
    exec(&mut vm, "CREATE TABLE tw3(id INTEGER PRIMARY KEY, tw2_id INTEGER, val INTEGER)");
...
fn test_explain_select() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE exp(id INTEGER PRIMARY KEY, val TEXT)");
...
fn test_explain_with_index() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE exp_idx(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_show_indexes() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE si_t(id INTEGER PRIMARY KEY, a TEXT, b INTEGER)");
...
fn test_vm_file_based_operations() {
    ...
    
    {
        ...
        exec(&mut vm, "CREATE TABLE fb_t(id INTEGER PRIMARY KEY, val TEXT)");
...
fn test_vm_file_based_multi_table() {
    ...
    
    {
        ...
        exec(&mut vm, "CREATE TABLE mt2(id INTEGER PRIMARY KEY, ref_id INTEGER)");
...
fn test_null_in_aggregates() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE na(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "INSERT INTO na VALUES (1, 10), (2, NULL), (3, 30), (4, NULL)");
    let rows = query_rows(&mut vm, "SELECT SUM(val), AVG(val), COUNT(val), COUNT(*) FROM na");
...
fn test_integer_real_comparison() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE irc(id INTEGER PRIMARY KEY, i INTEGER, r REAL)");
    exec(&mut vm, "INSERT INTO irc VALUES (1, 10, 10.0), (2, 10, 10.5), (3, 10, 9.5)");
...
fn test_union_with_aggregation() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE ua1(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE TABLE ua2(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_intersect_query() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE int1(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE TABLE int2(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "INSERT INTO int1 VALUES (1, 10), (2, 20), (3, 30)");
    exec(&mut vm, "INSERT INTO int2 VALUES (1, 20), (2, 30), (3, 40)");
    
    let rows = query_rows(&mut vm, "SELECT val FROM int1 INTERSECT SELECT val FROM int2");
...
fn test_except_query() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE exc1(id INTEGER PRIMARY KEY, val INTEGER)");
    exec(&mut vm, "CREATE TABLE exc2(id INTEGER PRIMARY KEY, val INTEGER)");
...
fn test_like_case_insensitive() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE lci(id INTEGER PRIMARY KEY, name TEXT)");
    exec(&mut vm, "INSERT INTO lci VALUES (1, 'Alice'), (2, 'ALICE'), (3, 'alice'), (4, 'Bob')");
...
fn test_not_like() {
    let mut vm = VM::new_memory();
    exec(&mut vm, "CREATE TABLE nl(id INTEGER PRIMARY KEY, name TEXT)");
    exec(&mut vm, "INSERT INTO nl VALUES (1, 'hello'), (2, 'world'), (3, 'help')");
