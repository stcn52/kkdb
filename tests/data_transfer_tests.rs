...
fn test_backup_and_restore() {
    ...
    // Initial Database
    {
        ...
        vm.execute_sql("INSERT INTO users VALUES (1, 'Alice');")
            .unwrap();
        vm.execute_sql("INSERT INTO users VALUES (2, 'Bob, the builder');")
            .unwrap();
        vm.execute_sql("INSERT INTO users VALUES (3, 'O''Connor');")
            .unwrap(); // test quote escaping
    ...
    // Restore Database
    {
        ...
        if let ExecResult::QueryResult { rows, .. } = res {
            ...
            assert!(
                matches!(&rows[1][0], kkdb::types::Value::Text(t) if t.as_ref() == "Bob, the builder")
            );
...
fn test_export_and_import() {
...

    {
        ...
        vm.execute_sql("CREATE TABLE items (id INTEGER PRIMARY KEY, desc TEXT, val REAL);")
            .unwrap();
        vm.execute_sql("INSERT INTO items VALUES (1, 'Item 1', 10.5);")
            .unwrap();
        vm.execute_sql("INSERT INTO items VALUES (2, 'Item, with comma', 20.0);")
            .unwrap();
        vm.execute_sql("INSERT INTO items VALUES (3, 'Item \"with quotes\"', 30.0);")
            .unwrap();
        vm.execute_sql("INSERT INTO items VALUES (4, NULL, NULL);")
            .unwrap();
...

    {
        ...
        vm2.execute_sql("CREATE TABLE items (id INTEGER PRIMARY KEY, desc TEXT, val REAL);")
            .unwrap();
        vm2.import_csv(csv_path, "items").unwrap();

        // Verify Data
        let res = vm2
            .execute_sql("SELECT desc, val FROM items ORDER BY id;")
            .unwrap();
        if let ExecResult::QueryResult { rows, .. } = res {
            assert_eq!(rows.len(), 4);
            assert!(matches!(&rows[0][0], kkdb::types::Value::Text(t) if t.as_ref() == "Item 1"));
            assert!(
                matches!(&rows[1][0], kkdb::types::Value::Text(t) if t.as_ref() == "Item, with comma")
            );
            assert!(
                matches!(&rows[2][0], kkdb::types::Value::Text(t) if t.as_ref() == "Item \"with quotes\"")
            );
