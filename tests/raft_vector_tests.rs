...
async fn test_raft_vector_insert_replicates() {
    ...
    assert!(
        raft_write(
            leader,
            "CREATE VECTOR INDEX idx_v ON vecs(v) DIM 3 DISTANCE COSINE"
        )
        .await
    );
    assert!(
...
fn test_vec_ef_search_session_var() {
    ...
    vm.execute_sql("CREATE TABLE embs (id INTEGER PRIMARY KEY, v BLOB)")
        .unwrap();
    for (i, vec) in [
        ...
        vm.execute_sql(&format!(
            "INSERT INTO embs VALUES ({i}, {})",
            vec_expr(&vec)
        ))
        .unwrap();
    }
    vm.execute_sql("CREATE VECTOR INDEX idx ON embs(v) DIM 3 DISTANCE COSINE")
        .unwrap();
    ...
    let rows = match vm
        .execute_sql(&format!(
            "SELECT id, {vs} AS score FROM embs ORDER BY {vs} DESC LIMIT 1"
        ))
        .unwrap()
    {
    ...
    let rows2 = match vm
        .execute_sql(&format!(
            "SELECT id, {vs2} AS score FROM embs ORDER BY {vs2} DESC LIMIT 1"
        ))
        .unwrap()
    {
        ExecResult::QueryResult { rows, .. } => rows,
...
async fn test_raft_single_node_vector_write() {
    ...
    assert!(raft_write(&node, "CREATE TABLE t (id INTEGER PRIMARY KEY, emb BLOB)").await);
    assert!(raft_write(&node, "CREATE VECTOR INDEX idx_t ON t(emb) DIM 2").await);
    assert!(
        raft_write(
            &node,
            &format!("INSERT INTO t VALUES (1, {})", vec_expr(&[0.5, 0.5]))
        )
        .await
    );
    assert!(
        raft_write(
            &node,
            &format!("INSERT INTO t VALUES (2, {})", vec_expr(&[0.1, 0.9]))
        )
        .await
    );
