...
use kkdb::server::http_api::{AppState, build_router};
...
async fn test_rls_isolation() {
    ...
    let (_, b1) = post_json(&r, "/auth/signup",
        json!({"email": "user1@test.com", "password": "pw1"}), None).await;
    ...
    let (_, b2) = post_json(&r2, "/auth/signup",
        json!({"email": "user2@test.com", "password": "pw2"}), None).await;
    ...
    let _ = post_json(&r3, "/rest/batch", json!({
        "statements": [
    ...
    let (_, qb2) = post_json(&r6, "/rest/query",
        json!({"sql": "SELECT * FROM items"}),
        Some((&auth2.0, &auth2.1))).await;
...
async fn test_batch_basic() {
    ...
    // Signup to get a token
    let r = make_router(&state);
    let (_, sb) = post_json(&r, "/auth/signup",
        json!({"email": "batch@test.com", "password": "pw"}), None).await;
...
async fn test_batch_transaction_rollback() {
    ...
    let r4 = make_router(&state);
    let (_, qb) = post_json(&r4, "/rest/query",
...
async fn test_bulk_write_basic() {
    ...
    let r = make_router(&state);
    let (_, sb) = post_json(&r, "/auth/signup",
        json!({"email": "bulk@test.com", "password": "pw"}), None).await;
    let token = sb["token"].as_str().unwrap().to_string();
    let auth = ("Authorization", format!("Bearer {}", token));

    // Pre-create table via HTTP API (lands in user's isolated VM)
    let r2 = make_router(&state);
    let _ = post_json(&r2, "/rest/execute",
...
async fn test_bulk_write_rollback_on_dup() {
    ...
    let r = make_router(&state);
    let (_, sb) = post_json(&r, "/auth/signup",
        json!({"email": "bulkrb@test.com", "password": "pw"}), None).await;
    let token = sb["token"].as_str().unwrap().to_string();
    let auth = ("Authorization", format!("Bearer {}", token));
