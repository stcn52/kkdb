...
use kkdb::raft::http_transport::{WalMetrics, RaftMetricsJson};
...
fn dummy_metrics(is_leader: bool, dead_records: u64) -> RaftMetricsJson {
    RaftMetricsJson {
        node_id: 1,
        role: if is_leader { "Leader".into() } else { "Follower".into() },
        ...
        wal: WalMetrics {
            ...
            dead_records,
            compaction_ratio_pct: if dead_records + 12 > 0 { dead_records * 100 / (dead_records + 12) } else { 0 },
        },
    }
}
...
fn test_metrics_json_serialization() {
    ...
    assert!(json.contains("\"last_log_index\":42"), "last_log_index present");
    assert!(json.contains("\"live_records\":12"), "WAL live records present");
    assert!(json.contains("\"membership_voter_ids\":[1,2,3]"), "voter list present");
}
...
fn test_prometheus_output_contains_all_metrics() {
    ...
    let metrics = [
        ("raft_is_leader", if m.current_leader == Some(m.node_id) { 1u64 } else { 0 }),
        ...
        ("raft_snapshot_last_log_index", m.snapshot_last_log_index.unwrap_or(0)),
        ...
        ("membership_voter_count", m.membership_voter_ids.len() as u64),
    ];
    ...
    for expected in &expected_metrics {
        assert!(out.contains(expected), "Prometheus output must contain {expected}");
    }
}
...
fn test_metrics_follower_not_leader() {
    ...
    let is_leader_val = if m.current_leader == Some(m.node_id) { 1u64 } else { 0 };
