...
fn assert_query_result(vm: &mut VM, sql: &str, expected_rows: Vec<Vec<Value>>) {
    match vm.execute_sql(sql).unwrap() {
        ExecResult::QueryResult { rows, .. } => {
            assert_eq!(rows.len(), expected_rows.len(), "Row count mismatch for: {}", sql);
            for (i, (actual, expected)) in rows.iter().zip(expected_rows.iter()).enumerate() {
                assert_eq!(actual.len(), expected.len(), "Row {} column count mismatch for: {}", i, sql);
                for (j, (a, e)) in actual.iter().zip(expected.iter()).enumerate() {
                    assert_eq!(format!("{}", a), format!("{}", e), "Row {} col {} mismatch for: {}", i, j, sql);
                }
            }
        }
...
fn test_schema_evolution() {
    ...
    // Create, Alter, Drop
    vm.execute_sql("CREATE TABLE emp (id INTEGER PRIMARY KEY, name TEXT);").unwrap();
    vm.execute_sql("INSERT INTO emp VALUES (1, 'Alice');").unwrap();
    ...
    // Create Index
    vm.execute_sql("CREATE INDEX idx_emp_name ON emp (name);").unwrap();
...
fn test_complex_aggregations() {
    ...
    vm.execute_sql("CREATE TABLE sales (id INTEGER PRIMARY KEY, region TEXT, amount REAL);").unwrap();
    vm.execute_sql("INSERT INTO sales VALUES (1, 'North', 100.0);").unwrap();
    vm.execute_sql("INSERT INTO sales VALUES (2, 'North', 150.0);").unwrap();
    vm.execute_sql("INSERT INTO sales VALUES (3, 'South', 200.0);").unwrap();
    vm.execute_sql("INSERT INTO sales VALUES (4, 'South', 50.0);").unwrap();
    vm.execute_sql("INSERT INTO sales VALUES (5, 'East', 300.0);").unwrap();
...
fn test_multi_table_transaction() {
    ...
    vm.execute_sql("CREATE TABLE accounts (id INTEGER PRIMARY KEY, balance INTEGER);").unwrap();
    vm.execute_sql("CREATE TABLE log (tx INTEGER PRIMARY KEY, msg TEXT);").unwrap();
    
    vm.execute_sql("INSERT INTO accounts VALUES (1, 1000);").unwrap();
    vm.execute_sql("INSERT INTO accounts VALUES (2, 500);").unwrap();
    
    // Successful transaction
    vm.execute_sql("BEGIN;").unwrap();
    vm.execute_sql("UPDATE accounts SET balance = balance - 200 WHERE id = 1;").unwrap();
    vm.execute_sql("UPDATE accounts SET balance = balance + 200 WHERE id = 2;").unwrap();
    vm.execute_sql("INSERT INTO log VALUES (1, 'Transfer 200');").unwrap();
    ...
    vm.execute_sql("BEGIN;").unwrap();
    vm.execute_sql("UPDATE accounts SET balance = balance - 100 WHERE id = 1;").unwrap();
    vm.execute_sql("UPDATE accounts SET balance = balance + 100 WHERE id = 2;").unwrap();
    vm.execute_sql("INSERT INTO log VALUES (2, 'Transfer 100 failed');").unwrap();
...
fn test_views_and_triggers() {
    ...
    vm.execute_sql("CREATE TABLE audit (id INTEGER PRIMARY KEY, old_val INTEGER, new_val INTEGER);").unwrap();
    ...
    vm.execute_sql("UPDATE t1 SET val = 20 WHERE id = 1;").unwrap();
    ...
    // View
    vm.execute_sql("CREATE VIEW v1 AS SELECT id, val * 2 AS doubled FROM t1;").unwrap();
    assert_query_result(
        &mut vm,
...
fn test_type_coercion_and_nulls() {
    ...
    vm.execute_sql("CREATE TABLE data (id INTEGER, a TEXT, b REAL);").unwrap();
    vm.execute_sql("INSERT INTO data VALUES (1, '100', 50.5);").unwrap();
    vm.execute_sql("INSERT INTO data VALUES (2, NULL, 10.0);").unwrap();
...
fn test_subqueries_and_ctes() {
    ...
    vm.execute_sql("CREATE TABLE orders (id INTEGER PRIMARY KEY, user_id INTEGER, amount REAL);").unwrap();

    vm.execute_sql("INSERT INTO users VALUES (1, 'Alice');").unwrap();
    vm.execute_sql("INSERT INTO users VALUES (2, 'Bob');").unwrap();
    
    vm.execute_sql("INSERT INTO orders VALUES (101, 1, 50.0);").unwrap();
    vm.execute_sql("INSERT INTO orders VALUES (102, 1, 100.0);").unwrap();
    vm.execute_sql("INSERT INTO orders VALUES (103, 2, 20.0);").unwrap();
...
fn test_large_data_scale() {
    ...
    vm.execute_sql("CREATE TABLE bulk_data (id INTEGER PRIMARY KEY, text_val TEXT, num_val INTEGER);").unwrap();
    vm.execute_sql("CREATE INDEX idx_bulk ON bulk_data (num_val);").unwrap();
    ...
    for i in 1..=2000 {
        ...
        vm.execute_sql(&format!("INSERT INTO bulk_data VALUES ({}, 'Some relatively long text payload {}', {});", i, i, i % 100)).unwrap();
    }
    vm.execute_sql("COMMIT;").unwrap();
