...
fn test_bm25_create_and_match_basic() {
    ...
    v.execute_sql(
        "INSERT INTO articles VALUES (2, 'Python Scripting', 'Python is great for scripting');",
    )
    .unwrap();
    v.execute_sql(
        "INSERT INTO articles VALUES (3, 'Rust vs Go', 'Comparing Rust and Go performance');",
    )
    .unwrap();
    ...
    let rows = rows_from(
        v.execute_sql("SELECT title FROM articles WHERE articles MATCH 'rust';")
            .unwrap(),
    );
    assert_eq!(
        rows.len(),
        2,
        "Expected 2 rows matching 'rust', got: {:?}",
        rows
    );
...
fn test_bm25_no_match() {
    let mut v = vm();
    v.execute_sql("CREATE TABLE docs (id INTEGER PRIMARY KEY, content TEXT);")
        .unwrap();
    v.execute_sql("INSERT INTO docs VALUES (1, 'Hello world');")
        .unwrap();
    v.execute_sql("INSERT INTO docs VALUES (2, 'Goodbye world');")
        .unwrap();
    v.execute_sql("CREATE FULLTEXT INDEX idx_docs ON docs (content);")
        .unwrap();

    let rows = rows_from(
        v.execute_sql("SELECT content FROM docs WHERE docs MATCH 'kkdb';")
            .unwrap(),
    );
...
fn test_bm25_unicode_query() {
    ...
    v.execute_sql("INSERT INTO books VALUES (1, '数据库引擎设计');")
        .unwrap();
    v.execute_sql("INSERT INTO books VALUES (2, 'Database Engine Design');")
        .unwrap();
    v.execute_sql("INSERT INTO books VALUES (3, '分布式存储系统');")
        .unwrap();
    ...
    let rows = rows_from(
        v.execute_sql("SELECT title FROM books WHERE books MATCH '数据库';")
            .unwrap(),
    );
    assert_eq!(
        rows.len(),
        1,
        "Expected 1 row with '数据库' token, got: {:?}",
        rows
    );
...
fn test_bm25_insert_after_create_index() {
    ...
    v.execute_sql("INSERT INTO news VALUES (2, 'PostgreSQL adds vector search');")
        .unwrap();
    v.execute_sql("INSERT INTO news VALUES (3, 'BM25 is better than TF-IDF');")
        .unwrap();
    ...
    assert_eq!(
        rows2.len(),
        2,
        "Expected 2 rows for 'search', got: {:?}",
        rows2
    );
}
...
fn test_bm25_delete_removes_from_index() {
    ...
    assert_eq!(
        after.len(),
        0,
        "Deleted row should not appear, got: {:?}",
        after
    );
}
