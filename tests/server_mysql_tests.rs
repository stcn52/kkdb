...
async fn test_mysql_server_integration() {
    ...
    let opts =
        mysql_async::Opts::from_url(&format!("mysql://root:password@127.0.0.1:{}/test", port))
            .unwrap();
    let mut conn = Conn::new(opts).await.unwrap();

    // 1. DDL
    conn.query_drop("CREATE TABLE test_users (id INTEGER PRIMARY KEY, name TEXT)")
        .await
        .unwrap();

    // 2. DML
    conn.query_drop("INSERT INTO test_users VALUES (1, 'Alice'), (2, 'Bob')")
        .await
        .unwrap();
    ...
    let result2: Vec<String> = conn
        .query("SELECT name FROM test_users WHERE id = (SELECT MAX(id) FROM test_users)")
        .await
        .unwrap();
