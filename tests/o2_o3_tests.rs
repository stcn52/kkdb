...
fn test_o2_analyze_enables_cbo() {
    ...
    vm.execute_sql("CREATE TABLE nums (id INTEGER PRIMARY KEY, val INTEGER);")
        .unwrap();
    // Insert 100 rows
    for i in 1..=100i64 {
        vm.execute_sql(&format!("INSERT INTO nums VALUES ({}, {});", i, i * 10))
            .unwrap();
    }
    vm.execute_sql("CREATE INDEX idx_nums_val ON nums (val);")
        .unwrap();
    vm.execute_sql("ANALYZE TABLE nums;").unwrap();
    // Query with high selectivity (1 out of 100) — CBO should choose index
    let r = rows(
        vm.execute_sql("SELECT id FROM nums WHERE val = 500;")
            .unwrap(),
    );
...
fn test_o2_cbo_prefers_seq_scan_for_large_range() {
    ...
    vm.execute_sql("CREATE TABLE t (id INTEGER PRIMARY KEY, x INTEGER);")
        .unwrap();
    for i in 1..=20i64 {
    ...
    let r = rows(
        vm.execute_sql("SELECT COUNT(*) FROM t WHERE x > 5;")
            .unwrap(),
    );
    assert_eq!(r.len(), 1);
...
fn test_o2_without_stats_still_uses_index() {
    ...
    vm.execute_sql("CREATE TABLE items (id INTEGER PRIMARY KEY, code TEXT);")
        .unwrap();
    vm.execute_sql("INSERT INTO items VALUES (1, 'ABC');")
        .unwrap();
    vm.execute_sql("INSERT INTO items VALUES (2, 'DEF');")
        .unwrap();
    vm.execute_sql("CREATE INDEX idx_items_code ON items (code);")
        .unwrap();
    // No ANALYZE — CBO falls back to always using index when available
    let r = rows(
        vm.execute_sql("SELECT id FROM items WHERE code = 'ABC';")
            .unwrap(),
    );
    assert_eq!(r.len(), 1);
...
fn test_o3_adaptive_index_auto_created() {
    ...
    for i in 1..=10 {
        vm.execute_sql(&format!(
            "INSERT INTO events VALUES ({}, 'cat_{}')",
            i,
            i % 3
        ))
        .unwrap();
    ...
    for _ in 0..3 {
        let _ = vm
            .execute_sql("SELECT id FROM events WHERE category = 'cat_1';")
            .unwrap();
    }
    ...
    assert!(
        index_exists,
        "O3 should have auto-created index on events.category"
    );
}
...
fn test_o3_no_duplicate_index() {
    ...
    vm.execute_sql("CREATE TABLE products (id INTEGER PRIMARY KEY, name TEXT);")
        .unwrap();
    vm.execute_sql("CREATE INDEX idx_products_name ON products (name);")
        .unwrap();
    vm.execute_sql("INSERT INTO products VALUES (1, 'Widget');")
        .unwrap();
    vm.adaptive_threshold = 1;
    // Even after many queries, no auto-index should be duplicated
    for _ in 0..5 {
        let _ = vm
            .execute_sql("SELECT id FROM products WHERE name = 'Widget';")
            .unwrap();
    }
    ...
    assert!(
        !auto_idx,
        "Should NOT create auto-index when manual index already exists"
    );
}
...
fn test_o3_counter_tracks_per_column() {
    ...
    for _ in 0..3 {
        let _ = vm
            .execute_sql("SELECT id FROM log WHERE level = 'INFO';")
            .unwrap();
    }
    // Access `msg` column only 1 time
    let _ = vm
        .execute_sql("SELECT id FROM log WHERE msg = 'hello';")
        .unwrap();
    let level_count = vm
        .query_access_counter
        .get(&("log".to_string(), "level".to_string()))
        .copied()
        .unwrap_or(0);
    let msg_count = vm
        .query_access_counter
        .get(&("log".to_string(), "msg".to_string()))
        .copied()
        .unwrap_or(0);
