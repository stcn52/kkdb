...
fn test_check_constraints_column_level() {
    ...
    vm.execute_sql("CREATE TABLE t1 (id INTEGER PRIMARY KEY, age INTEGER CHECK (age >= 18))")
        .unwrap();

    // Valid insert
    vm.execute_sql("INSERT INTO t1 (id, age) VALUES (1, 18)")
        .unwrap();
    vm.execute_sql("INSERT INTO t1 (id, age) VALUES (2, 20)")
        .unwrap();

    // Invalid insert
    let err = vm
        .execute_sql("INSERT INTO t1 (id, age) VALUES (3, 17)")
        .unwrap_err();
    assert!(matches!(err, KkdbError::ConstraintViolation(_)));

    // Valid update
    vm.execute_sql("UPDATE t1 SET age = 19 WHERE id = 1")
        .unwrap();

    // Invalid update
    let err = vm
        .execute_sql("UPDATE t1 SET age = 15 WHERE id = 2")
        .unwrap_err();
    assert!(matches!(err, KkdbError::ConstraintViolation(_)));

    // NULL passes CHECK dynamically (SQL standard)
    vm.execute_sql("INSERT INTO t1 (id, age) VALUES (4, NULL)")
        .unwrap();
}
...
fn test_check_constraints_table_level() {
    ...
    // 2. Table-level CHECK
    vm.execute_sql(
        "CREATE TABLE t2 (start_val INTEGER, end_val INTEGER, CHECK (start_val <= end_val))",
    )
    .unwrap();
    ...
    // Invalid insert
    let err = vm
        .execute_sql("INSERT INTO t2 VALUES (30, 20)")
        .unwrap_err();
    assert!(matches!(err, KkdbError::ConstraintViolation(_)));
...
fn test_load_schema_preserves_check() {
    let dir = tempfile::tempdir().unwrap();
    {
        ...
        vm.execute_sql("CREATE TABLE t1 (x INTEGER CHECK (x > 0))")
            .unwrap();
